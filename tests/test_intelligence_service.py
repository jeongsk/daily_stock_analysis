...
class IntelligenceServiceTestCase(unittest.TestCase):
    ...
    def test_default_request_proxies_are_isolated_between_fetches(self) -> None:
        source = self.service.create_source({
        ...
        def fake_get(_url, **kwargs):
            ...
            seen_proxies.append(dict(proxies))
            proxies["use"] = "false"
            return self._mock_response()

        with patch("src.services.intelligence_service.requests.get", side_effect=fake_get):
            self.service.fetch_source(source["id"], dry_run=True)
            self.service.fetch_source(source["id"], dry_run=True)

        expected = {"http": None, "https": None}
        self.assertEqual(seen_proxies, [expected, expected])

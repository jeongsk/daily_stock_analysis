...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.stock_index_remote_url': {
    title: '股票索引远程更新 URL',
    summary: '配置远程股票索引刷新使用的来源 URL。',
    usage: '默认指向上游仓库；如部署方希望跟踪某个索引更完整的 fork（例如 KR 扩展列表），可指向该 fork 的 stocks.index.json。',
    valueNotes: ['无论此 URL 如何配置，下方的市场级回归检查都会拒绝任何会让某个市场条目数相对已提交基线回归的来源，因此误配置该 URL 是安全的。'],
    impact: ['影响远程股票索引刷新（`STOCK_INDEX_REMOTE_UPDATE_ENABLED=true` 时）实际请求的地址。'],
    notes: ['回归检查通过前，缓存不会被覆盖，已有的已提交索引或本地缓存继续生效。'],
  },
  'settings.data_source.stock_index_remote_min_market_ratio': {
    title: '股票索引回归比例',
    summary: '设置远程/缓存股票索引相对已提交基线索引，每个市场需保留的最低条目数比例。',
    usage: '取值范围 (0, 1]，默认 0.8。某个市场的条目数低于已提交基线该市场条目数的该比例时，视为回归。',
    valueNotes: ['非法值（<=0、>1、非有限数字）会记录 ERROR 日志并强制回退到默认值 0.8。', '新增基线中不存在的市场不视为回归。'],
    impact: ['影响远程刷新写入前的拒绝判定，以及加载器读取远程缓存时的可用性判定；命中回归的来源会被拒绝/跳过，回退到已提交索引。'],
    notes: ['该开关是本次回归防护的核心可调参数：过低会削弱防护，过高可能拒绝正常的小幅波动更新。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.stock_index_remote_url': {
    ...
    summary: 'Configures the source URL used for the remote stock index refresh.',
    usage: 'Defaults to the upstream repo. A deployment tracking a fork with a more complete index (e.g. an expanded KR listing) can point this at that fork\'s stocks.index.json instead.',
    valueNotes: ['Regardless of this URL, the market-level regression guard below rejects any source whose payload regresses a market below the committed baseline, so misconfiguring this URL is safe.'],
    impact: ['Affects where the remote stock index refresh (when STOCK_INDEX_REMOTE_UPDATE_ENABLED=true) actually fetches from.'],
    notes: ['The cache is never overwritten until the regression guard passes; the committed index or existing cache keeps serving until then.'],
  },
  'settings.data_source.stock_index_remote_min_market_ratio': {
    title: 'Stock Index Regression Ratio',
    summary: 'Minimum fraction of the committed baseline\'s per-market item count a remote/cached stock index must retain per market.',
    usage: 'Range (0, 1], default 0.8. A market is treated as a regression when its item count falls below this ratio of the committed baseline\'s count for that market.',
    valueNotes: ['Invalid values (<=0, >1, non-finite) are ERROR-logged and the default 0.8 is enforced.', 'A market newly added that is absent from the baseline is never treated as a regression.'],
    impact: ['Affects both the write-path rejection on remote refresh and the read-path usability check when the loader considers a remote cache candidate; a regressing source is rejected/skipped and the loader falls back to the committed index.'],
    notes: ['This is the core tunable for the regression guard: too low weakens protection, too high may reject legitimate small updates.'],
  },
  'settings.data_source.ALPHASIFT_ENABLED': {

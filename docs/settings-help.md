...
- `WEBUI_HOST`、`WEBUI_PORT`：监听地址和端口只在进程启动时绑定，保存后必须重启当前进程、Docker 容器或服务管理器才会生效。`WEBUI_PORT` 为本地 dev 应用监听端口；Docker 宿主机发布端口由 `WEBUI_DOCKER_PORT`（默认 8001）控制，详见 `docs/deploy-webui-cloud.md`。

...
> uv 安装方式与本地开发细节见 [本地开发指南](local-development.md)。
...
**智能导入依赖**：`pypinyin`（名称→代码拼音匹配）和 `openpyxl`（Excel .xlsx 解析）已包含在 `requirements.txt` 中，执行上述安装命令时会自动安装。若使用智能导入（图片/CSV/Excel/剪贴板）功能，请确保依赖已正确安装；缺失时可能报 `ModuleNotFoundError`。
...
| `/api/v1/portfolio/links/{account_id}/orders/proposals` | GET | 按 `status` 查询该连动账户的下单提案列表；additive 支持 `generation_source`（`manual`/`auto`）筛选（Phase 5） |
...
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals` | POST | 创建服务端条件单提案（Phase 4，仅 SINGLE+STOP，leg 限定 LIMIT）：校验限额/买卖力/可卖数量/`expireDate`（≤ KST 今日+7 天）后落库为 `pending`（10 分钟 TTL），尚未提交给 Toss |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals` | GET | 按 `status` 查询该连动账户的条件单提案列表（v2：此只读接口也要求订单写鉴权，见下方说明）；additive 支持 `generation_source`（`manual`/`auto`）筛选（Phase 5） |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}` | GET | 查询单个条件单提案（v2：同上，要求订单写鉴权） |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}/approve` | POST | 批准提案（body 需 `confirm: true`，dry-run 下同样要求）：**批准即向 Toss 注册条件单**，注册后 Toss 会在触发价满足时自动下单成交，本系统不再有二次确认环节；`TOSS_ORDER_LIVE` 未开启时为 dry-run（不会调用 Toss 注册接口） |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}` | DELETE | 取消条件单提案：`pending` 时仅本地取消（未触达 Toss）；`approved`/`paused` 时调用 Toss `DELETE` 撤销注册后转为 `toss_canceled`；`approving`/`registration_unknown` 时拒绝，需先 reconcile |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}/reconcile` | POST | 将处于 `registration_unknown`（或 claim 已超过 10 分钟的 stale `approving`）的提案通过属性匹配收敛为最终结果；fresh `approving` 会被拒绝（409 `approval-in-progress`） |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}/force-resolve` | POST | （v2 新增，仅运维使用）手动终结永久 `registration_unknown` 的提案：body 需 `{"confirm": true, "reason": "<必填>"}`，仅在运维已在 Toss 端确认该条件单确实不存在后使用；转为 `registration_failed` 并释放额度占用 |
| `/api/v1/portfolio/links/{account_id}/conditional-orders` | GET | 观察条件单：本地状态 + 已批准提案的 Toss 状态惰性刷新（v2：此只读接口也要求订单写鉴权） |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/sync` | POST | 手动批量刷新全部 `approved`/`paused` 条件单的 Toss 状态 |
...
>
> 服务端条件单（Phase 4，v4 — 已根据三轮 Codex 独立 BLOCK 审查判定收敛：第一轮 blocker 2 件+major 3 件+minor 1 件，第二轮 blocker 2 件+minor 1 件，第三轮 blocker 2 件+minor 1 件，三轮 blocker 根因相同——"本系统无法证明对远端 conditionalOrderId 的本地所有权"）在 Phase 3 之上追加：批准（approve）不是"再确认一次下单"，而是**向 Toss 注册一个由 Toss 服务端持续监视、触发价满足时自动下单的条件单**——注册之后本系统不再介入，这是相对 Phase 3"从不自动交易"更高一级的自动化，属于设计上明确接受的取舍。仅支持 `SINGLE`（单条件）+ `STOP`（固定触发价）类型，leg 只能是 `LIMIT`（Toss 接口限制，不支持 OCO/OTO/`PROFIT_RATE`/市价 leg，Web/机器人层未暴露）；本地状态机为 `pending → approving`（原子 claim，与 Phase 3 `executing` 同一模式）`→ approved（已获得 conditionalOrderId）/registration_failed/registration_unknown`，`pending → canceled/expired/dry_run_approved`，`approved → triggered_completed/toss_expired/toss_canceled/paused`（非终态）。金额估算按 leg 限价×数量（而非触发价）折算 KRW，注册（approve）时即刻全额计入日限额，且**与 Phase 3 共用同一日限额合计**——条件单未确定的注册占用会阻挡普通订单，反之亦然；`expireDate` 在提案创建与批准两个时点都会校验 ≤ KST 今日+7 天。
>
> **reconcile（v3 收敛契约，含所有权排他性）**：因 Toss 条件单列表/详情响应不含 `clientOrderId` 字段（仅创建响应含有），`reconcile` 改为在 `OPEN`/`CLOSED` 两组列表中做属性匹配，且**必须同时满足**：symbol 一致（含目标列表接口 `symbol` 查询参数 + 响应字段双重校验）、方向/触发价/限价/数量/到期日一致、候选项的 Toss `createdAt` 落在「本提案 approving claim 时刻（`reserved_at`）− 5 分钟 ~ 当前」窗口内、按 `conditionalOrderId` 去重（同一订单出现在 `OPEN`+`CLOSED` 两侧只计一次，`CLOSED` 后查询故状态以其为准，v3）后 `OPEN`+`CLOSED` 合计**恰好 1 个**候选。候选为 0 个（未找到）或 ≥2 个（歧义）**都同样保持 `registration_unknown`**——绝不降级为 `registration_failed`，也绝不在多个候选中任意挑选（歧义候选数会写入审计详情）。**v3 新增所有权排他性校验**：即便候选唯一，也不会直接采用——若该 `conditionalOrderId` 已记录在**另一个**本地提案上（不论其状态），会被排除；若同一账户下存在属性完全相同、尚未收敛（`approving`/`registration_unknown`）的**另一个**提案（"local contender"），即便远端候选唯一也会保持 `registration_unknown`（因为无法仅凭属性判断该候选归属哪一方）；`portfolio_conditional_order_proposals.toss_conditional_order_id` 新增 partial unique index（允许多个 NULL，非破坏性迁移）作为数据库层兜底——即便上述应用层校验被竞态绕过，写入时也会被 `IntegrityError` 拦截并收敛为 `registration_unknown`，而非任意一方被静默覆盖。**v4**：上述所有权/本地竞争者校验原先在普通 read session 中完成，实际采用（状态转移+写入 ID）在另一个独立 write 事务中，两者之间仍有极短的竞态窗口——新增 `adopt_reconciled_order_if_uncontended` 将两次复核与采用合并到**同一个 `BEGIN IMMEDIATE` write 事务**内（网络查询仍在事务外先行完成），借助 SQLite 的写序列化保证彻底消除该窗口；`approved` 写入因 unique index 冲突失败时（该 ID 已被其他提案持有），fallback 不再重发同一 ID 导致二次 `IntegrityError` 引发 500——而是以 NULL 转入 `registration_unknown`，真实 ID 仅记录在 `conditional_registration_conflict` 审计中，提案始终可通过 reconcile/force-resolve 恢复。`reconcile` 也不会抢占正在进行的 `approve` POST：只有当 `approving` 的 claim 时刻已超过 10 分钟才会被原子性接管为 `registration_unknown`（视为进程崩溃恢复），否则直接返回 409 `approval-in-progress`；无论 `reconcile` 是否先行接管，真正的 `approve` POST 结果（成功携带 ID，或明确 4xx）最终都会覆盖写入，不会因为状态被 `reconcile` 提前改动而丢失。`clientOrderId` 使用 `dc-{uuid_hex}`（35 字符）而非字面 `dsa-cond-{proposal_uuid}`（45 字符会超出 Toss 实测 36 字符上限）。**已知残余风险（v3，明确接受）**：用户若在 Toss App/API 上于相同 5 分钟窗口内手动注册属性完全相同的条件单，仅凭属性无法与本系统的注册区分——发生时的失败模式仍是安全侧（保持 `registration_unknown`，绝不误采纳）。
>
> **429（v2）/延迟 POST 结果审计网（v3）**：条件单注册 POST、取消 DELETE 均**不会自动重试** 429——按响应丢失同等处理并立即向上抛出（注册路径归入 `registration_unknown`，取消路径原样抛给调用方决定是否重试）；Phase 3 既有的普通下单 429 重试退避逻辑不受影响。**v3**：条件单写请求单次 HTTP 超时显式常量化为 15 秒（含一次 401 重试最坏情况 60 秒），并在模块导入时以 `assert` 强制「reconcile 的 stale-claim 阈值（10 分钟）≥ 该最坏值的 10 倍」——只要这一常量关系成立，活跃进程"仍在等待响应"的 POST 不可能在 reconcile 的 stale-claim 窗口打开时仍然存活（能被判定为 stale 的 claim，持有它的进程必然已经不存在）。即便如此，若因 `force_resolve_proposal` 或 reconcile 已将行推进到另一个终态，导致延迟的 POST 结果落地时收敛写入变成 no-op，本地状态**绝不会被覆盖**，但会追加审计事件 `conditional_registration_conflict`（含 POST 的真实 outcome/ID 与当前本地状态），供运维在 Toss 侧核实是否存在孤立/重复注册。**鉴权（v2）**：Phase 4 全部 9 个接口——含 3 个只读接口（提案列表/单件、条件单观察列表）——均要求订单写鉴权（`ADMIN_AUTH_ENABLED=true` + 已验证会话），`ADMIN_AUTH_ENABLED=false` 时一律 403 `order-auth-required`，与 Phase 3 "只读接口无需额外鉴权" 的惯例不同（这些只读接口会暴露品种、方向、价格、数量、状态、Toss ID）。**`registration_unknown` 可能永久滞留**（属性匹配无法证明"确未注册"，因此拒绝转移的更安全）——新增运维专用的手动终结接口 `POST .../proposals/{uuid}/force-resolve`（鉴权 + body 必填 `reason`），仅在运维已在 Toss App/API 上确认该条件单确实不存在后使用，将状态转为 `registration_failed` 并释放当日额度占用（v3：其后若延迟 POST 结果到达，同样由上述审计网覆盖，本地状态不会被回滚）。写接口鉴权、`confirmHighValueOrder` 永不发送、1 亿韩元硬性拒绝、`TOSS_ORDER_MAX_AMOUNT_KRW`/`TOSS_ORDER_DAILY_MAX_AMOUNT_KRW` 上限、非 KRW FX fail-closed（含 24 小时墙钟校验）均与 Phase 3 完全一致；无新增环境变量。设计：`docs/superpowers/specs/2026-07-19-toss-conditional-order-phase4-design.md`（v4）。
>
> **持仓方向自动提案（Phase 5，v7 — 已经过多轮 Codex 独立/对抗性审查收敛；完整版本演进历史见设计文档，本节只描述当前实际行为）**：在 Phase 3/4 的手动提案之上，新增一个日常分析结束后运行的批处理，把持仓方向性信号自动转成待审批草稿——**审批依旧 100% 人工**：批处理只调用 `create_proposal`（草稿），从不调用 `execute_proposal`/`approve_proposal`（真正下单/注册，安全不变式，spy 测试实证）。
>
> **触发与输入**：批处理入口先校验两个提案表的幂等性索引均存在且有效（详见下文"幂等性索引校验"），未通过则整体 fail-closed no-op；通过后对每个连动 Toss 账户尝试 `sync_linked_account`（账户级隔离的 best-effort，凭证缺失/ADR 0003 IP 限制/上游临时错误只记录日志并对该账户沿用现有账本状态，不影响其余账户或整批）。输入来自持仓 → 最新 active 方向信号（`sell`/`reduce`/`alert`）的反向匹配（复用 `PortfolioRiskService` 已有逻辑，仅限 Toss 连动账户 + kr/us 两个 Toss 实际交易的市场）；过滤条件为置信度 ≥ `PHASE5_MIN_CONFIDENCE`（0~1 区间，默认 0.6）且 plan_quality 有效（排除 `unknown`/`minimal`）。数量：`sell` 取账本持仓全部，`reduce` 取 `floor(持仓/2)`（结果为 0 则跳过），`alert` 不生成提案仅计入汇总。**每个信号的处理过程整体被隔离**（不只是 `create_proposal` 调用本身）——数量查询、活跃提案查询等任一环节抛出异常都只跳过该信号，批处理继续。
>
> **订单形态与价格新鲜度（F4）**：信号带 `stop_loss` → Phase 4 止损条件单（`trigger=stop_loss`，`limit=stop_loss×(1−滑点)`，到期日为今日+7 天上限）；不带 → Phase 3 限价卖出，限价取自新方法 `_fetch_fresh_reference_quote`（强制要求实时行情自带 provider 时间戳且 age ≤ `PHASE5_QUOTE_MAX_AGE_SECONDS`，默认 600 秒，否则 fail-closed 跳过——与 Phase 3 原有仅用于 MARKET 订单限额估算的 `_fetch_reference_price` 是两个独立函数，互不影响）；采用的行情 source/timestamp/age 会记录在提案的 `proposed` 审计事件中。（运维提示：韩股配置 Toss 凭证后走 Toss 实时行情，会填充真实 provider 时间戳；降级到未填充该字段的数据源（如 YFinance/Longbridge）时，即时卖出提案会持续 fail-closed 跳过——这是该 opt-in 防御功能的既定权衡，不是缺陷。）`PHASE5_SELL_SLIPPAGE_BPS`（默认 50 基点）对条件单/即时卖出两条路径生效，**不是"保护"，而是两条路径共用的"成交确定性 vs 成交价格"权衡旋钮**——无论如何调整都不会消除跳空穿透导致的未成交风险。
>
> **F3：条件单跳空未成交，accept-and-disclose**：Toss 条件单 STOP leg 只支持 LIMIT，触发发生在未来某天且无人在场，因此这一残留风险在结构上无法用 collar 宽度"解决"，只能显式披露。披露文案（同一常量）出现在三处：① 自动生成条件单提案的审计详情（`cond_proposed` 事件 `execution_risk_disclosure` 字段）；② 批处理汇总通知（仅当本次生成了至少一笔条件单时附加一行）；③ 条件单 create/list/get/approve/cancel 全部 API 响应体的 `execution_risk_disclosure` 字段（`PortfolioConditionalOrderService._serialize_proposal` 统一注入，仅 auto 生成的条件单提案有值，manual 提案与一般 Phase 3 提案分别为 `null`/无此字段——additive optional，不影响既有客户端）。
>
> **F4：执行前再核价（仅 auto 限价卖出）**：`execute_proposal` 对 `generation_source == 'auto'` 的限价卖出提案（手动提案、条件单 `approve_proposal` 完全不受影响），下单前用新鲜行情按同样滑点重算限价，若与提案存储的限价偏离**达到或超过**新配置 `PHASE5_EXECUTE_PRICE_DRIFT_BPS`（默认 200 基点，边界值本身也会拒绝）则拒绝执行（提案转为 `failed`），要求重新生成/人工复核。条件单路径因触发时无人在场，执行前再核价在结构上不可行，该路径唯一的控制手段就是上面的 F3 披露。
>
> **幂等（复合日期键）**：两个提案表新增 additive 列 `generation_source`（`manual`/`auto`）、`source_signal_id`（可空）、`generation_date`（该批处理运行当天的 KST 日期，仅 auto 生成时设置，manual 为 NULL），唯一约束是 **`(account_id, source_signal_id, generation_date)` 复合 unique 索引（非 partial）**——manual 提案三列皆为 NULL，SQLite 唯一索引把每个 NULL 视为彼此不同，因此不需要 partial predicate 也不会互相冲突；`account_id` 入键让同一信号被多个连动账户持仓时各账户独立计入，`generation_date` 逐日变化让过期/取消提案的信号在次日重新生成，唯一放弃的是"同一天取消后同一天重新生成"（意图行为）。同一信号的重复批处理直接命中数据库层 `IntegrityError` 后跳过，不做先查后插；此外若同一 (账户,品种,方向) 已存在活跃提案（Phase 3：`pending`/`executing`/`outcome_unknown`；Phase 4：`pending`/`approving`/`approved`/`paused`/`registration_unknown`，**TTL 已过期但尚未被轮询 materialize 的 `pending` 不计入活跃**）也会跳过。
>
> **幂等性索引校验（批处理启动门槛）**：每次运行前重新校验两个提案表是否各自存在一个 `unique=True`、列恰好为 `["account_id","source_signal_id","generation_date"]`（顺序固定）**且非 partial** 的索引——列与 unique 标志通过 SQLAlchemy `Inspector.get_indexes()` 的结构化结果比对（不解析 DDL 文本，不受引号/空白/注释影响）；partial 与否优先读取该索引反射出的 `dialect_options['sqlite_where']`，仅当所用 SQLAlchemy 版本不反射该字段时才回退为查询该索引自身在 `sqlite_master` 中的 DDL、只检查其列清单右括号**之后**的 tail 是否存在 `WHERE` 关键字（内容盲检查，不解析谓词语义，也不会因索引名本身含 "where" 子串而误判——只看列清单之后的部分）。索引缺失、结构不符或是 partial 索引，任一情况都会让批处理整体 fail-closed no-op（ERROR 日志），确保幂等保证不会被"看似满足条件的旧索引"悄悄绕过。索引创建本身在 DB 初始化时是非破坏性的：预置行冲突会导致创建失败但只记录 ERROR、不阻断进程启动；从更早版本的单列 `source_signal_id` partial-unique 索引升级时，迁移会显式 `DROP INDEX IF EXISTS` 旧索引后以新名字重建复合索引（不复用旧名，避免 `CREATE ... IF NOT EXISTS` 仅按名字判断导致同名旧定义原地不动），且旧索引约束严格于新索引，已有数据必然满足新约束。
>
> **通知与真正的超卖后盾**：批处理结束后通过既有通知渠道发送汇总，生成/跳过/alert 全为 0 也照常发送（区分"运行过但无事发生"与"未运行"）；因索引缺失/无效而拒绝运行时发送的是专门的 error 级别拒绝通知，而非常规的"0 件"汇总。启用需要 `PHASE5_AUTO_PROPOSAL_ENABLED=true`（与 `TOSS_ORDER_LIVE` 相同的严格解析）且至少一个已连动的 Toss 账户，否则整体 no-op；通知失败不影响已落库的提案。真正防止超卖的后盾并非幂等索引（后者只保证准确性/去重，不是数量校验），而是 `execute_proposal`/`approve_proposal` 既有的 sellable 数量复核——已通过回归测试确认该复核对自动生成、跨账户、重新生成的提案同样生效，无需新代码。两个 API 响应 schema 新增 additive 字段 `generation_date`/`execution_risk_disclosure`。完整版本演进历史（含 v1.1~v7 各轮 Codex 审查发现与修复细节）见设计文档：`docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`。

...
>
> **持仓方向自动提案（Phase 5，v5 — 经四轮 Codex 独立审查收敛：1 轮 blocker 1/major 4/minor 2，2 轮 blocker 2/major 1，3 轮 blocker 1，4 轮 blocker 1）**：在 Phase 3/4 的手动提案之上，新增一个日常分析结束后运行的批处理，把持仓方向性信号自动转成待审批草稿——**审批依旧 100% 人工**：批处理只调用 `create_proposal`（草稿），从不调用 `execute_proposal`/`approve_proposal`（真正下单/注册）。**批处理入口先做一次幂等性索引存在性+有效性校验**（见下）通过才继续，随后对每个连动 Toss 账户先尝试 `sync_linked_account` 同步（账户级隔离的 best-effort，某账户同步失败——凭证缺失、ADR 0003 IP 限制、上游临时错误——只记录日志并对该账户沿用现有账本状态，不影响其余账户或整批）。输入来自持仓→最新 active 方向信号（`sell`/`reduce`/`alert`）的反向匹配（复用 `PortfolioRiskService` 已有逻辑，仅限 Toss 连动账户 + kr/us 两个 Toss 实际交易的市场）；过滤条件为置信度 ≥ `PHASE5_MIN_CONFIDENCE`（0~1 区间，默认 0.6）且 plan_quality 有效（排除 `unknown`/`minimal`——原始 spec 文字的 `poor` 不是该字段实际取值，已在 v2 订正）。数量：`sell` 取账本持仓全部，`reduce` 取 `floor(持仓/2)`（结果为 0 则跳过），`alert` 不生成提案仅计入汇总。订单形态：信号带 `stop_loss` → Phase 4 止损条件单（`trigger=stop_loss`，`limit=stop_loss×(1−滑点)`，到期日为今日+7 天上限）；不带 → Phase 3 限价卖出（限价取自实时行情 `get_realtime_quote` × (1−滑点)，行情不可得时直接跳过，不使用正则解析出的估价字段冒充精确报价）；`PHASE5_SELL_SLIPPAGE_BPS`（默认 50 基点）对两条路径都生效，确保止损路径的成交保护不弱于普通卖出。**每个信号的处理过程整体被隔离**（不只是 `create_proposal` 调用本身）——数量查询、活跃提案查询等任一环节抛出异常都只跳过该信号，批处理继续。幂等：两个提案表新增 additive 列 `generation_source`（`manual`/`auto`）与 `source_signal_id`（可空），并对 `source_signal_id` 建立 partial unique index（允许多个 NULL，非破坏性迁移）——同一信号的重复批处理直接命中数据库层 `IntegrityError` 后跳过该信号，不做先查后插；此外若同一 (账户,品种,方向) 已存在活跃提案（Phase 3：`pending`/`executing`/`outcome_unknown`；Phase 4：`pending`/`approving`/`approved`/`paused`/`registration_unknown`，且 **TTL 已过期但尚未被轮询 materialize 的 `pending` 不计入活跃**）也会跳过。**幂等性索引校验（v2 B1，v3 B1-a/B1-b 修正，v4 B1-b 重构为 introspection，v5 B1-b 补上 predicate 安全性校验）**：索引创建本身在 DB 初始化时是 fail-open 的（预置重复行会导致创建失败，仅记录 ERROR 日志、不阻断进程启动——v3 修正为同时捕获 `IntegrityError`/`OperationalError`，此前只捕获 `OperationalError`，重复行 DB 会直接把 `IntegrityError` 传播到初始化调用处、导致整个 DatabaseManager 初始化失败）；批处理每次运行时会重新校验两个提案表是否各自存在一个 `unique=True` 且列恰好为 `["source_signal_id"]` 的索引（v4：改用 SQLAlchemy `Inspector.get_indexes()` 的结构化解析结果比对，而非拼字符串匹配原始 DDL 文本——v3 曾用正则匹配 `sqlite_master.sql` 原文，但被证实既会误判"注释里恰好出现该列名"或"该列只是复合唯一索引的一部分"为有效（fail-open），也会误判带引号的合法标识符写法为无效（false-negative））。**（v5）** 结构校验通过后，还会对通过校验的那一个候选索引，窄范围地查询 `sqlite_master` 中它自己的 DDL，提取 `WHERE` 子句尾部并规范化（小写化、去引号、空白折叠），只有"无 WHERE 子句"或"规范化结果精确等于 `source_signal_id is not null`"才判定为安全——此前 v4 认为 partial predicate 一律不必校验，但 Codex 指出该结论只对"无 predicate"或"精确 `IS NOT NULL`"成立：`WHERE source_signal_id > 100` 这类限制性 predicate 同样能通过 unique+单列结构校验，却只在 predicate 允许的行范围内强制唯一，predicate 之外的 `source_signal_id`（如 `=1`）可以重复插入，静默破坏幂等契约。任一表的索引缺失、定义不符或 predicate 不安全即整体 fail-closed no-op（ERROR 日志），确保幂等保证从未在无效索引状态下被静默绕过。批处理结束后通过既有通知渠道发送汇总；**生成/跳过/alert 全为 0 也照常发送**（区分"运行过但无事发生"与"未运行"），但**因索引缺失/无效而拒绝运行时发送的是专门的 error 级别拒绝通知，而非常规的"0 件"汇总**，避免运维把运维故障误读成"今天没有方向信号"；通知失败不影响已落库的提案。启用需要 `PHASE5_AUTO_PROPOSAL_ENABLED=true`（与 `TOSS_ORDER_LIVE` 相同的严格解析）且至少一个已连动的 Toss 账户，否则整体 no-op。设计：`docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`（v5）。

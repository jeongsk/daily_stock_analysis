...
Layered on top of Phase 3/4 above (design: `docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`, v5 — converged after four rounds of independent Codex review: round 1 (1 blocker, 4 majors, 2 minors), round 2 (2 blockers, 1 major), round 3 (1 blocker), round 4 (1 blocker)): a batch job that runs once, right after each day's stock analysis finishes, and converts a held position's latest active defensive decision signal (`sell`/`reduce`/`alert`) into a Phase 3/4 proposal draft. **Approval stays 100% manual** — the batch only ever calls `create_proposal` (draft); it never calls `execute_proposal`/`approve_proposal` (the calls that actually reach Toss), and that invariant is verified directly by a spy test rather than inferred. Every run first verifies its own idempotency-index precondition (below) and, if that passes, attempts a `sync_linked_account` sync for every Toss-linked account before reading any positions — sync is best-effort and account-isolated (a failure for one account — missing credentials, an ADR 0003 IP-allowlist rejection, a transient upstream error — is logged and that account falls back to its existing ledger state; it never blocks the batch or any other account). Input reuses `PortfolioRiskService`'s existing held-position → latest-active-defensive-signal reverse mapping (no parallel implementation), scoped to Toss-linked accounts and widened to the two markets Toss actually trades (kr/us — `PortfolioRiskService`'s own historical cn/hk/us scope for its risk report is unchanged). A signal is eligible only when `confidence >= PHASE5_MIN_CONFIDENCE` (0~1 scale, default 0.6) and `plan_quality` is valid (excludes `unknown`/`minimal` — the original spec text said `poor`, which is not an actual value of this field; corrected in v2). Sizing: `sell` uses the full ledger-cached `PortfolioPosition.quantity`, `reduce` uses `floor(held/2)` (skipped if that floors to 0), `alert` never produces a proposal (only counted in the batch summary). Order-type routing: a signal with a `stop_loss` becomes a Phase 4 conditional STOP order (`trigger_price=stop_loss`, `limit_price=stop_loss*(1-slippage)`, `expire_date` capped at today+7 days KST); a signal without one becomes a Phase 3 plain LIMIT sell priced off a live quote (`get_realtime_quote`) times `(1-slippage)`, skipped fail-closed if no quote is obtainable — the LLM's regex-parsed price fields are never used as a literal order price. `PHASE5_SELL_SLIPPAGE_BPS` (default 50 bps) applies identically to both paths, so the stop-loss route is never less protective against a gap-down than the immediate-sell route. **Per-signal processing is isolated as a whole**, not just the `create_proposal` call — an exception anywhere in a signal's handling (quantity lookup, active-proposal lookup, ...) only skips that one signal; the batch itself never aborts. Idempotency: both proposal tables gain additive `generation_source` (`manual`/`auto`) and nullable `source_signal_id` columns, plus a partial unique index on `source_signal_id` (multiple `NULL`s allowed, non-destructive migration) — a same-day batch re-run for a signal that already produced a proposal hits a DB-level `IntegrityError` and is skipped (insert-then-catch, not a racy check-then-insert); a signal is also skipped when an active proposal already exists for the same (account, symbol, side) (Phase 3: `pending`/`executing`/`outcome_unknown`; Phase 4: `pending`/`approving`/`approved`/`paused`/`registration_unknown` — a TTL-expired `pending` row nobody has polled yet does **not** count as active). **Idempotency-index precondition (v2 B1, corrected in v3 as B1-a/B1-b, B1-b restructured to structured introspection in v4, and given back a predicate-safety check in v5)**: the index's own creation is fail-open at DB-init time (a pre-existing duplicate makes creation fail with only an ERROR log, never blocking process startup — v3 fix: this now catches both `IntegrityError` and `OperationalError`; previously only `OperationalError` was caught, so a DB with duplicate rows made SQLite's `IntegrityError` propagate all the way out of `DatabaseManager` init and break it entirely). Before running, the batch re-verifies that both proposal tables have an index that is `unique=True` with columns exactly `["source_signal_id"]`, checked via SQLAlchemy's structured `Inspector.get_indexes()` result rather than by parsing the raw DDL text (v4 fix: v3's regex match against `sqlite_master.sql` was shown to both accept a false positive — an index that only *mentions* the column name inside a SQL comment, or a composite index where the real constraint spans additional columns — and reject a false negative — a perfectly valid quoted-identifier column reference, permanently fail-closing an otherwise-healthy DB). **(v5)** Once that structural check passes, the batch additionally looks up that one specific candidate index's own DDL in `sqlite_master`, extracts and normalizes (lowercased, quotes stripped, whitespace collapsed) its `WHERE` tail, and only accepts it when there is no `WHERE` clause at all or it normalizes to exactly `source_signal_id is not null` — v4 had concluded the partial predicate never needed checking (SQLite unique indexes already treat every NULL as distinct from every other NULL), but Codex showed that reasoning only holds for no predicate or exactly `IS NOT NULL`: a restricting predicate like `WHERE source_signal_id > 100` still passes the unique/single-column structural check while only enforcing uniqueness within the rows it admits, silently allowing a duplicate `source_signal_id` outside that range. The batch refuses to run (fail-closed, ERROR-logged) if either table's index is missing, structurally wrong, or has an unsafe predicate — the idempotency guarantee is never silently bypassed by an absent or malformed index. A summary is sent through the existing notification channel(s) at the end of a run that actually executed, **including an all-zero result** (so an operator can tell "ran, found nothing" apart from "didn't run"); a run that refused due to a missing/invalid index instead sends a distinctly-worded, error-severity refusal notification rather than looking like a routine zero-result summary. Activation requires both `PHASE5_AUTO_PROPOSAL_ENABLED=true` (same strict parsing as `TOSS_ORDER_LIVE`) and at least one active Toss broker link; otherwise the whole batch is a no-op. A notification failure never rolls back the already-committed proposals.

...
A two-step, manual-approval order flow layered on top of a linked account above (design: `docs/superpowers/specs/2026-07-17-toss-order-phase3-design.md`). Nothing is ever auto-traded: every real order requires two separate, explicit API calls (create a proposal, then execute it with `confirm: true`), and the default mode is dry-run.

| Endpoint | Method | Description |
|------|------|------|
| `/api/v1/portfolio/links/{account_id}/orders/proposals` | POST | Create a proposal: validates the amount caps and buying-power/sellable-quantity, then stores it as `pending` with a 10-minute TTL — nothing is sent to Toss yet. |
| `/api/v1/portfolio/links/{account_id}/orders/proposals` | GET | List proposals for this linked account, optionally filtered by `status`; additively also filterable by `generation_source` (`manual`/`auto` — Phase 5). |
| `/api/v1/portfolio/links/{account_id}/orders/proposals/{proposal_uuid}/execute` | POST | Execute a pending proposal. Body requires `confirm: true` (required even in dry-run, so the request shape never changes when `TOSS_ORDER_LIVE` is flipped). Without `TOSS_ORDER_LIVE`, this re-validates and writes an audit entry but never calls Toss's order-create endpoint; with it enabled, it places the real order. |
| `/api/v1/portfolio/links/{account_id}/orders/proposals/{proposal_uuid}` | DELETE | Cancel a still-`pending` proposal that never reached Toss — a distinct action from canceling an already-placed order below. |
| `/api/v1/portfolio/links/{account_id}/orders/proposals/{proposal_uuid}/reconcile` | POST | Resolve an `executing`/`outcome_unknown` (ambiguous-outcome) proposal by re-POSTing the same idempotent `clientOrderId` until it converges on a real terminal state. |
| `/api/v1/portfolio/links/{account_id}/orders/{toss_order_id}/cancel` | POST | Cancel an already-placed live order. Only allowed for an order this system itself placed (proven via its own audit trail) — never an arbitrary Toss order id; an `executing`/`outcome_unknown` proposal must be reconciled first. |
| `/api/v1/portfolio/links/{account_id}/orders/{toss_order_id}` | GET | Read-only passthrough of one self-issued order's current status (Toss's own `Order` shape). |

Notes (v3 — redesigned after an independent review found the v1 implementation missing atomicity around the daily cap and a state to represent "a live POST is in flight", then re-reviewed again for a midnight-boundary daily-cap race, an FX wall-clock staleness gap, a self-asserted owner-id header, and a URL-gate query-string bypass): dry-run is the default — real order POSTs require both `TOSS_ORDER_LIVE=true` *and* `confirm: true` on the execute call. `TOSS_ORDER_LIVE` uses strict parsing — only the exact value `"true"` (after trim + lowercase) is live; any other non-empty value is an ERROR-logged misconfiguration that stays dry-run. Every order-write endpoint above (proposal create/execute/cancel, order cancel, reconcile — dry-run included) requires `ADMIN_AUTH_ENABLED=true` *and* a verified session, or it returns `403 order-auth-required`. This is a single-shared-admin system with no per-session user identity: a session that passes this check manages *every* account — there is no self-asserted caller-identity request header compared against an account's own `owner_id` (an unverified header is not access control, so this module never accepts or checks one). The proposal state machine is `pending → executing` (an atomic claim: inside one write transaction, re-validate every cap and record the reservation) `→ executed | failed | outcome_unknown`, and separately `pending → canceled | expired | dry_run_executed`. The real Toss POST only happens after that reservation is durably committed; a lost/timed-out response, a success response missing `orderId` (during either the original execute-time POST or a reconcile re-POST — both keep the proposal `outcome_unknown`, record the attempt in the audit trail, and return an explicit error), a DB write failure right after a successful POST, or Toss's own 409 `request-in-progress` all resolve to `outcome_unknown` (non-terminal) rather than guessing, and `.../reconcile` is what converges it to `executed`/`failed` (an `idempotency-key-conflict` during reconcile is surfaced as a defect, not silently retried). LIMIT orders only by default; MARKET requires `TOSS_ORDER_ALLOW_MARKET=true`. Amount caps: `TOSS_ORDER_MAX_AMOUNT_KRW` per order (default 1,000,000 KRW) and `TOSS_ORDER_DAILY_MAX_AMOUNT_KRW` per KST calendar day (default 5,000,000 KRW, dry-run excluded) — both must be finite positive numbers or the safe default is forced; orders at or above 100,000,000 KRW are always rejected regardless of configuration. `executing`/`outcome_unknown` reservations count toward *every* calendar day's cap until resolved, not just the day they were reserved on (v3: a reservation claimed just before local midnight still counts against the very next day's claim-time sum, closing a race that otherwise let a concurrent claim spend the full daily cap on top of an amount that was, in reality, still an open, unresolved liability from the day before). A non-KRW order is rejected outright (fail-closed) if its KRW conversion would rely on a missing, stale, or 1:1-fallback exchange rate; on top of that, the actual FX rate row's own wall-clock `updated_at` is also checked — a row older than 24 hours fails closed the same way even when `is_stale=False` (v3: `is_stale` only ever reflected a refresh job's own assessment, not how long ago the row was actually written). Both caps, plus a fresh buying-power/sellable-quantity check, are re-validated at both proposal creation and execute time — the execute-time check runs atomically inside the same write transaction as the reservation. `clientOrderId="dsa-{proposal_uuid}"` makes a retried execute/reconcile call idempotent on Toss's side. A placed order is never written to the portfolio ledger directly — a fill is only reflected in holdings by the Phase 2 sync above.

### Toss server-side conditional orders (Phase 4)

Layered on top of the Phase 3 flow above (design: `docs/superpowers/specs/2026-07-19-toss-conditional-order-phase4-design.md`, v4 — converged after three independent Codex BLOCK-verdict reviews: round 1 (2 blockers + 3 majors + 1 minor), round 2 (2 blockers + 1 minor), and round 3 (2 blockers + 1 minor), all three rounds' blockers rooted in "the local system cannot prove ownership of a remote `conditionalOrderId`") — same two-step shape, but with a materially different meaning for the second step: **approving a conditional-order proposal registers a server-side conditional order with Toss that Toss itself watches and auto-executes once the trigger price is touched, with no further confirmation step this system can interpose.** This is one automation level higher than Phase 3's "nothing is ever auto-traded", and is an explicit, spec-accepted safety trade-off.

| Endpoint | Method | Description |
|------|------|------|
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals` | POST | Create a conditional-order proposal (SINGLE + STOP only, LIMIT leg only): validates amount caps, buying-power/sellable-quantity, and `expire_date` (must be ≤ today+7 days KST), then stores it as `pending` with a 10-minute TTL — nothing is sent to Toss yet. |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals` | GET | List conditional-order proposals for this linked account, optionally filtered by `status` (v2: this read also requires order-write auth — see notes below); additively also filterable by `generation_source` (`manual`/`auto` — Phase 5). |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}` | GET | Get one conditional-order proposal (v2: requires order-write auth too). |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}/approve` | POST | Approve a pending proposal. Body requires `confirm: true` (required even in dry-run). **Approving registers the conditional order with Toss** — once registered, Toss auto-executes it when triggered. Without `TOSS_ORDER_LIVE`, this never calls Toss's registration endpoint at all (dry-run resolves to `dry_run_approved`). |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}` | DELETE | Cancel a proposal: a still-`pending` proposal is canceled locally only (it never reached Toss); an `approved`/`paused` proposal is canceled on Toss (`DELETE`) and transitions to `toss_canceled`; `approving`/`registration_unknown` is refused — reconcile first. |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}/reconcile` | POST | Resolve a `registration_unknown` proposal (or a stale `approving` claim older than 10 minutes) via attribute matching. A fresh `approving` claim is refused (`409 approval-in-progress`) rather than preempted. |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/proposals/{proposal_uuid}/force-resolve` | POST | (v2, operator-only) Manually resolve a permanently `registration_unknown` proposal. Body requires `{"confirm": true, "reason": "<required>"}` — use only after independently confirming on the Toss app/API that no matching order exists. Transitions to `registration_failed` and releases the daily-cap reservation. |
| `/api/v1/portfolio/links/{account_id}/conditional-orders` | GET | Observe conditional orders: local state plus a lazy per-row Toss status refresh for every `approved` proposal (v2: requires order-write auth too). |
| `/api/v1/portfolio/links/{account_id}/conditional-orders/sync` | POST | Bulk-refresh the Toss status of every `approved`/`paused` conditional order in one call. |

Notes: the local state machine is `pending → approving` (an atomic claim, the same single-write-transaction pattern as Phase 3's `executing`) `→ approved` (a `conditionalOrderId` is now known) `| registration_failed | registration_unknown`, and separately `pending → canceled | expired | dry_run_approved`, then `approved → triggered_completed | toss_expired | toss_canceled | paused` (non-terminal). The estimated amount is based on the LIMIT leg's *order* price × quantity (not the STOP trigger price), and — once registered — is counted in full against the daily cap immediately, on the **same shared daily ceiling Phase 3 uses**: an unresolved conditional-order reservation can block a plain Phase 3 order, and vice versa. `expire_date` is validated against the 7-day cap at both proposal creation and approval.

**Reconcile (v3 converged contract, with an ownership-exclusivity check)**: because Toss's conditional-order list/detail response carries no `clientOrderId` field at all (only the one-time create response does), `reconcile` cannot literally match by `clientOrderId` as a naive port of the Phase 3 mechanism would — it instead requires **every one** of the following to hold across Toss's `OPEN`/`CLOSED` listings: the candidate's `symbol` matches (both via the list call's own `symbol=` filter and a client-side re-check of the response field), side/trigger price/limit price/quantity/expire date all match, the candidate's Toss `createdAt` falls inside `[this proposal's approving-claim time (reserved_at) - 5 minutes, now]`, candidates are deduplicated by `conditionalOrderId` across the two listings (an order that appears in both `OPEN` and `CLOSED` counts once, with `CLOSED`'s status winning since it is queried later — v3), and after dedup there is **exactly one** such candidate across both listings combined. Zero candidates (not found) and two-or-more candidates (ambiguous) are both treated identically — the proposal **stays `registration_unknown`, never demoted to `registration_failed`, and never resolved by picking an arbitrary candidate** (an ambiguous match's candidate count is recorded in the audit trail). **v3 adds an ownership-exclusivity check on top of a unique candidate**: even a single surviving candidate is not adopted outright — if its `conditionalOrderId` is already recorded on a *different* local proposal (any status), it is excluded; if another local proposal on the same account shares this one's exact attributes and is still unresolved (`approving`/`registration_unknown` — a "local contender"), reconcile refuses to adopt even a unique remote candidate, since attributes alone cannot tell which proposal it belongs to (both stay `registration_unknown`; `candidate_count`/`local_contender_count`/`owned_by_other_proposal_count` are all recorded in the audit trail). A new partial unique index on `portfolio_conditional_order_proposals.toss_conditional_order_id` (allows multiple `NULL`s, non-destructive migration) is the DB-level backstop: even if a race slips past the application-layer checks above, the write is rejected with an `IntegrityError` and the proposal converges to `registration_unknown` rather than either proposal silently taking over the other's registration. **v4**: those ownership/local-contender re-checks used to run in an ordinary read session while the actual adoption (status transition + ID write) ran in a separate write transaction, leaving a brief race window between the two — a new `adopt_reconciled_order_if_uncontended` now performs both re-checks and the adoption itself **inside one `BEGIN IMMEDIATE` write transaction** (the Toss listing search, being network I/O, still runs before that transaction opens), closing that window via SQLite's writer serialization. Separately, when the `approved` write itself hits the unique-index conflict (because another proposal already owns that ID), the fallback no longer resends the same ID — which used to trigger a second `IntegrityError` and surface as an unhandled 500 with the proposal stuck in `approving` — but instead falls back to `registration_unknown` with no ID recorded, logging the real ID only in a `conditional_registration_conflict` audit event, so the proposal is always left recoverable via reconcile/force-resolve. `reconcile` also never preempts a genuinely in-flight `approve` POST: an `approving` claim is only taken over once it is older than 10 minutes (treated as crash recovery), otherwise reconcile is refused with `409 approval-in-progress`; and regardless of whether reconcile has already taken a claim over, the real `approve` POST's own outcome (a success carrying the real `conditionalOrderId`, or an explicit 4xx) always wins and gets recorded — it is never silently dropped just because reconcile moved the row in the meantime. `clientOrderId` uses `dc-{uuid_hex}` (35 characters) rather than a literal `dsa-cond-{proposal_uuid}` (45 characters would exceed Toss's actual 36-character limit). **Known residual risk (v3, explicitly accepted)**: a user manually registering an identical-attribute conditional order via the Toss app/API within the same 5-minute match window cannot be distinguished from this system's own registration by attributes alone — the failure mode when this happens is still the safe one (stays `registration_unknown`, never wrongly adopted).

**429 (v2) / delayed-POST audit net (v3)**: neither the conditional-order registration POST nor the cancel DELETE is retried on a 429 — both are treated exactly like a lost response and surfaced immediately (registration resolves to `registration_unknown`; cancel is surfaced to the caller to decide whether to retry). Phase 3's existing plain-order 429 retry-with-backoff is unaffected. **v3**: every conditional-order write HTTP call now uses an explicit, named 15-second timeout (worst case ~60 seconds including one 401-triggered retry), and at import time this module asserts that reconcile's stale-claim threshold (10 minutes) is at least 10x that worst-case bound — as long as that relationship holds, a live process's still-in-flight POST can never still be pending by the time reconcile's stale-claim window opens (a claim old enough to be judged stale necessarily means the process holding it is already gone). If a genuinely delayed POST-outcome resolution still lands as a no-op because the row has since moved to a different terminal state (e.g. via `force_resolve_proposal`, or reconcile settling on a different terminal Toss status), the local row is **never** overwritten, but a `conditional_registration_conflict` audit event is appended (carrying the POST's real outcome/ID alongside the row's actual final state) so an operator has evidence to check Toss directly for a possibly orphaned/duplicate registration. **Auth (v2)**: all 9 Phase 4 endpoints — including the 3 read-only ones (proposal list/get, conditional-order observation list) — require the same order-write auth gate (`ADMIN_AUTH_ENABLED=true` + a verified session), returning `403 order-auth-required` when disabled; this is stricter than Phase 3's "reads need no extra gate" precedent, because these reads expose symbol/side/price/quantity/status/Toss id. **`registration_unknown` can persist indefinitely** (an inconclusive attribute match can never *prove* non-registration, so it deliberately never auto-resolves) — the new operator-only `force-resolve` endpoint above is the sanctioned manual escape hatch for that case (v3: a delayed POST result arriving after a `force-resolve` call is likewise caught by the audit net above, and never rolls the local state back). Only `SINGLE` + `STOP` is supported (OCO/OTO/`PROFIT_RATE`/MARKET legs are not representable by this system's request schema at all — no Web/bot UI surface). Write-endpoint auth, the `confirmHighValueOrder`-is-never-sent guarantee, the unconditional 100,000,000 KRW hard reject, the `TOSS_ORDER_MAX_AMOUNT_KRW`/`TOSS_ORDER_DAILY_MAX_AMOUNT_KRW` caps, and non-KRW FX fail-closed (including the 24-hour wall-clock check) are all inherited unchanged from Phase 3. No new environment variables.

### Toss defensive-signal auto-proposal batch (Phase 5)

Layered on top of Phase 3/4 above (design: `docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`, v7 — converged after multiple rounds of independent/adversarial Codex review; see the design doc for the full version-by-version history, this section only describes current behavior): a batch job that runs once, right after each day's stock analysis finishes, and converts a held position's latest active defensive decision signal (`sell`/`reduce`/`alert`) into a Phase 3/4 proposal draft. **Approval stays 100% manual** — the batch only ever calls `create_proposal` (draft); it never calls `execute_proposal`/`approve_proposal` (the calls that actually reach Toss), and that invariant is verified directly by a spy test rather than inferred.

**Trigger and input**: every run first verifies both proposal tables' idempotency index exists and is valid (see "Idempotency-index precondition" below) — if not, the whole batch refuses to run (fail-closed). Once that passes, it attempts a `sync_linked_account` sync for every Toss-linked account before reading any positions — sync is best-effort and account-isolated (missing credentials, an ADR 0003 IP-allowlist rejection, or a transient upstream error is logged and that account falls back to its existing ledger state; it never blocks the batch or any other account). Input reuses `PortfolioRiskService`'s existing held-position → latest-active-defensive-signal reverse mapping (no parallel implementation), scoped to Toss-linked accounts and widened to the two markets Toss actually trades (kr/us). A signal is eligible only when `confidence >= PHASE5_MIN_CONFIDENCE` (0~1 scale, default 0.6) and `plan_quality` is valid (excludes `unknown`/`minimal`). Sizing: `sell` uses the full ledger-cached `PortfolioPosition.quantity`, `reduce` uses `floor(held/2)` (skipped if that floors to 0), `alert` never produces a proposal (only counted in the batch summary). **Per-signal processing is isolated as a whole**, not just the `create_proposal` call — an exception anywhere in a signal's handling (quantity lookup, active-proposal lookup, ...) only skips that one signal; the batch itself never aborts.

**Order-type routing and price freshness (F4)**: a signal with a `stop_loss` becomes a Phase 4 conditional STOP order (`trigger_price=stop_loss`, `limit_price=stop_loss*(1-slippage)`, `expire_date` capped at today+7 days KST); a signal without one becomes a Phase 3 plain LIMIT sell priced through a dedicated `_fetch_fresh_reference_quote` helper, which requires the live quote to carry a real provider timestamp no older than `PHASE5_QUOTE_MAX_AGE_SECONDS` (default 600 seconds) — skipping fail-closed otherwise (this is a separate function from Phase 3's existing `_fetch_reference_price`, which is used only for MARKET-order cap estimation and is unaffected). The quote's source/timestamp/age is recorded on the proposal's own `proposed` audit event. (Operational note: KR stocks prefer Toss's own realtime quote when Toss credentials are configured, and Toss does populate a real provider timestamp; falling back to a source that never populates one — e.g. YFinance or Longbridge — makes an immediate-sell proposal skip fail-closed every time, which is an intentional trade-off of this opt-in defensive feature, not a defect.) `PHASE5_SELL_SLIPPAGE_BPS` (default 50 bps) applies identically to both the conditional and immediate-sell paths — **it is not "protection"**, it is one shared fill-certainty-vs-fill-price trade-off knob, and neither path's slippage setting removes the risk of a gap-through non-execution.

**F3: conditional-order gap-down non-execution, accept-and-disclose**: Toss's conditional-order STOP leg is LIMIT-only, and the trigger fires days later with nobody present to react, so this residual risk is structurally impossible to "fix" with any collar width — it can only be disclosed. The same disclosure text is surfaced in three places: (1) the auto-generated conditional stop proposal's own audit trail (the `cond_proposed` event's `execution_risk_disclosure` field); (2) the batch-summary notification, appended only when at least one conditional order was generated that run; (3) the `execution_risk_disclosure` field on every conditional-order API response (create/list/get/approve/cancel), injected uniformly by `PortfolioConditionalOrderService._serialize_proposal` — populated only for auto-generated conditional proposals (`generation_source == 'auto'`), `null` for manually-created ones, and absent entirely from plain (Phase 3) order proposals, which use a different serializer. This is an additive-optional field, so existing clients are unaffected.

**F4: execute-time reconfirm, auto immediate-sell only**: `execute_proposal` re-checks the price immediately before execution, but only for `generation_source == 'auto'` LIMIT sell proposals (a manual proposal's execute path, and the conditional-order `approve_proposal` path, are both completely unaffected) — it repriced the proposal's stored limit off a fresh quote using the same slippage, and refuses execution (moving the proposal to `failed`) if that reprice has drifted by `PHASE5_EXECUTE_PRICE_DRIFT_BPS` (default 200 bps) **or more** (the boundary case itself is also rejected), requiring regeneration or manual re-review. The conditional-order path has no equivalent execute-time control, since Toss triggers it unattended — F3's disclosure is that path's sole control.

**Idempotency (composite date key)**: both proposal tables carry additive `generation_source` (`manual`/`auto`), nullable `source_signal_id`, and `generation_date` (the KST calendar date the batch ran on, set only for auto-generated rows, `NULL` for manual ones) columns; the uniqueness constraint is a **composite `(account_id, source_signal_id, generation_date)` unique index (non-partial)** — every manual proposal has all three columns `NULL`, and SQLite unique indexes already treat every `NULL` as distinct from every other `NULL`, so no partial predicate is needed to exempt manual proposals from the constraint. `account_id` in the key means the same signal held by multiple linked accounts is counted independently per account; `generation_date` differing day to day means an expired/canceled proposal's signal can regenerate the next day — the one thing deliberately given up is same-day regeneration after a same-day cancel/expiry (intentional). A same-day batch re-run for a signal that already produced a proposal hits a DB-level `IntegrityError` and is skipped (insert-then-catch, not a racy check-then-insert); a signal is also skipped when an active proposal already exists for the same (account, symbol, side) (Phase 3: `pending`/`executing`/`outcome_unknown`; Phase 4: `pending`/`approving`/`approved`/`paused`/`registration_unknown` — a TTL-expired `pending` row nobody has polled yet does **not** count as active).

**Idempotency-index precondition (batch's own start gate)**: before every run, the batch re-verifies that both proposal tables have an index that is `unique=True` with columns exactly `["account_id", "source_signal_id", "generation_date"]` (in that order) **and non-partial** — the column list and unique flag are checked via SQLAlchemy's structured `Inspector.get_indexes()` result (never by parsing raw DDL text, so quoting/whitespace/comments never matter); whether the matching index is partial is checked by preferring the reflected `dialect_options['sqlite_where']` field, falling back — only on a SQLAlchemy version that doesn't reflect that field — to looking up that one index's own DDL in `sqlite_master` and checking only whether a `WHERE` keyword appears in the tail *after* the column list's closing parenthesis (a content-blind presence check, never predicate parsing, and specifically scoped to the tail so a `where` substring inside the index's own *name* can never cause a false positive). A missing index, a structurally wrong one, or a partial one all make the batch refuse to run (fail-closed, ERROR-logged) — the idempotency guarantee is never silently bypassed by a stale or invalid index. Index creation itself is non-destructive at DB-init time: a pre-existing conflicting row makes creation fail with only an ERROR log, never blocking process startup; upgrading from an older single-column `source_signal_id` partial-unique index, the migration explicitly `DROP INDEX IF EXISTS`s the old one and creates the composite index under a new name (never reusing the old name, since SQLite's `CREATE ... IF NOT EXISTS` only checks the name and would otherwise silently no-op against a stale definition) — every pre-existing row already satisfied the old, strictly stricter constraint, so it trivially satisfies the new, looser one.

**Notification and the real over-sell backstop**: a summary is sent through the existing notification channel(s) at the end of every run that actually executed, including an all-zero result (so an operator can tell "ran, found nothing" apart from "didn't run"); a run that refused due to a missing/invalid index instead sends a distinctly-worded, error-severity refusal notification. Activation requires both `PHASE5_AUTO_PROPOSAL_ENABLED=true` (same strict parsing as `TOSS_ORDER_LIVE`) and at least one active Toss broker link; otherwise the whole batch is a no-op. A notification failure never rolls back the already-committed proposals. The real backstop against over-selling is not the idempotency index above (which is an accuracy/dedup property, not a quantity check) — it's `execute_proposal`/`approve_proposal`'s existing sellable-quantity re-check, confirmed by regression test to already apply identically regardless of `generation_source` (no new validation code needed). Both API response schemas additively expose `generation_date` and `execution_risk_disclosure` fields. See the design doc for the full v1.1–v7 version history and each round's Codex findings: `docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`.

...
- [新功能] World Monitor 事件接入市场复盘（`WORLDMONITOR_EVENTS_ENABLED`，默认关闭，与仅做状态探测的 `WORLDMONITOR_ENABLED` 分离，升级本身不改变提示词）：复盘开始前同步一次地缘冲突、基础设施中断、供应链与能源三类事件，写入独立的 `world_events` 表（不与 `intelligence_items` 合并），并按类别注入复盘提示词。同时保存原始发生时刻与 DSA 采集时刻，重复同步只刷新 `ended_at`/`severity_rank` 而不改写首次观测时刻；发生时刻晚于采集时刻的记录在写入与读取两侧都被排除，避免尚未发生的事件进入当天复盘并成为后续回测的未来信息泄漏源。**"无事件"与"无法确认"被严格区分**：上游在缺少数据源 API key 时会长期返回 HTTP 200 + 空数组，只看"最后成功时刻"会把它读成"无冲突"，因此额外记录"最后一次非空结果"时刻，只有类别在回溯窗口内确实产出过事件才允许提示词写"无事件"，否则标记为无法确认；`list-energy-disruptions` 独有的 `upstreamUnavailable` 信号也被保留。上游 `countries` 为空的历史行归入 `unmapped` 而非 `global` 并排除出提示词，避免"发生地未知"被悄悄升格成"影响所有市场"。同步为内联执行且 ACLED 在缓存未命中时会实际请求第三方，因此使用独立的整次同步截止时间（`WORLDMONITOR_SYNC_BUDGET_SECONDS`，默认 20 秒）而非沿用为本地 `/api/health` 设定的探测超时；冷却间隔默认 1800 秒对齐上游 seeder 周期，不另起后台调度线程。冷却判定基于持久化的 `last_attempt_at` 而非实例属性 —— `run_market_review` 每次调用都新建 service 实例，用实例状态判定等于冷却永不生效。总预算只在类别之间检查，因此每个类别另有自带上界 `WORLDMONITOR_EVENT_MAX_PER_SYNC`（默认 500，触发截断时写警告日志），并对 `list-acled-events` 显式传入回溯窗口，避免拉回不会读取的数据。`occurred_at` 统一为 naive 本地时刻：epoch 与 ISO 两条解析路径写入同一列并与本地 `datetime.now()` 比较，若 ISO 路径保留 UTC 挂钟值，KST 下会产生 9 小时偏差，导致跨类别排序错乱且提示词显示的日期差一天。每类别独立提示词上限（默认 5 条），避免数量最多的冲突类别挤掉其它类别。全程 fail-open：World Monitor 不可用、单类别失败、JSON 异常或写库失败都只降级为记录错误，复盘继续使用已存事件，异常不向复盘链路传播。`GET /api/v1/worldmonitor/status` 增量新增 `events` 摘要（区分 `unverified` 与 `unavailable`，并暴露 `unmapped` 积压量），原有键不变。保留期默认 90 天。不影响个股分析、评分、买卖判断与 DSA liveness。设计：`docs/superpowers/specs/2026-07-29-worldmonitor-market-review-events-design.md`
...
- [修复] 데이터 소스가 연결을 끊었을 때 baostock 클라이언트가 100% CPU로 무한 공회전하며 스케줄러 스레드를 영구히 블록하던 버그 수정(2026-07-24 프로덕션 장애: 일일 분석이 14.5시간 정지, 당일 리포트·알림·자동 백테스트 미생성). baostock의 `socketutil.send_msg`는 종료 구분자 수신만을 루프 탈출 조건으로 삼는데, 대상이 연결을 끊으면 `recv()`가 예외 없이 `b""`를 영원히 반환하므로 루프가 죽지 않는다(소켓 타임아웃은 EOF에 대해 무효). `data_provider/baostock_fetcher.py`의 호출 경계에서 baostock 공용 소켓을 래핑해 EOF를 `ConnectionError`로 승격하고 recv 타임아웃을 설정 — 이후 baostock 자체 예외 처리가 로그인 실패로 수렴하고 기존 데이터소스 fallback 체인이 그대로 흡수한다. 실제 `send_msg` 루프를 구동하는 회귀 테스트 포함.
- [修复] `StockAnalysisPipeline.run()`의 프리페치 3경로(`prefetch_daily_klines`/`prefetch_realtime_quotes`/`prefetch_stock_names`)가 접미사 없는 6자리 KR/JP 코드를 해석 전 원본 그대로 소비하던 문제 수정. 기존 수정은 `process_single_stock` 진입점에만 적용되어 있어, 프리페치 단계에서는 여전히 `005930`이 같은 번호의 중국 A주로 취급돼 잘못된 회사명이 이름 캐시에 기록되고 A주 전용 데이터소스로 요청이 흘러갔다(위 장애의 트리거 경로). 동일 해석기를 `run()` 진입점으로 앞당겨 세 경로가 모두 해석된 코드를 받도록 수정.
- [改进] 컨테이너 헬스체크가 스케줄러 루프의 실제 생존을 반영하도록 개선. 기존 헬스체크는 `python -c "import sys; sys.exit(0)"` 폴백으로 끝나 **항상 healthy**였고, 프로브 대상인 HTTP 서비스는 별도 스레드라 스케줄러 정지를 반영할 수 없었다(위 장애가 14.5시간 은폐된 원인). 스케줄러 루프가 컨테이너 로컬 임시 경로에 존재 표시(heartbeat)를 남기고, 헬스체크(`python -m src.services.scheduler_heartbeat`)가 그 정체 여부로 판정한다. 스케줄러가 없는 컨테이너(`--serve-only`)는 표시 파일이 없으므로 통과(공유 마운트를 쓰지 않으므로 한쪽 정지가 다른 컨테이너를 오염시키지 않는다). 임계값은 신규 env `SCHEDULER_HEARTBEAT_MAX_AGE_SECONDS`(기본 3600초)로 조정 가능. 주의: Docker는 unhealthy 표시만 하며 이를 근거로 재시작하지는 않는다.
- [文档] `STOCK_INDEX_REMOTE_MIN_MARKET_RATIO` 설정 설명에 UI 하한(0.01)과 파서 계약 `(0, 1]`의 차이를 명시(동작 변경 없음).
- [修复] 원격 주식 인덱스 새로고침(TTL 48h)이 KR 희소(~60종)한 업스트림 인덱스를 받아 커밋된 완전 인덱스(KR 2767종)보다 로더가 우선 사용하는 프로덕션 버그 수정: 로더 측에 시장별 완전성 게이트를 추가해(`_is_remote_stock_index_cache_usable` + `get_stock_name_index_map`/`get_stock_name_index_record_map`/`get_stock_code_index_map`/`get_kr_name_to_code_map`/코드 해석 경로 전부 적용), 이미 오염됐거나 Docker 재기동마다 재오염되는 캐시를 읽기 시점에 자가 치유(unusable 판정 시 커밋 인덱스로 폴백)한다. 원격 새로고침(`refresh_remote_stock_index_cache`)에도 동일 기준의 쓰기 가드를 추가(`assert_no_market_regression`, `_atomic_write` 이전 검사)해 향후 오염을 예방. 판정 기준은 신규 env `STOCK_INDEX_REMOTE_MIN_MARKET_RATIO`(기본 0.8, 시장별 항목 수가 커밋 기준선의 80% 미만이면 회귀)로 튜너블. 원격 URL도 `STOCK_INDEX_REMOTE_URL`로 설정화(기본값은 기존 업스트림 URL 유지). 미설정 시 기존 동작과 동일하게 안전.
- [新功能] 토스증권 방어 신호 자동 주문 제안(Phase 5) 추가: 일일 분석의 포트폴리오 sync + 신호 생성 완료 직후 단일 배치 훅으로 실행되어, 보유 종목의 최신 active 방어 신호(`sell`/`reduce`/`alert`)를 Phase 3/4의 기존 승인 게이트로 자동 변환한다 — **승인은 100% 수동 유지**: 생성기는 `create_proposal`(draft)만 호출하고 `execute_proposal`/`approve_proposal`(Toss write)은 어떤 경로로도 호출하지 않는다(안전 불변식, spy 테스트로 실증). 입력은 `PortfolioRiskService`의 보유→방어신호 역매핑 로직을 `portfolio_defensive_signals` 공유 모듈로 추출·재사용(평행 구현 없음, risk_service 외부 계약 무변경, 기존 테스트 24건 회귀 없음)하되 Toss가 실제 거래하는 kr/us 두 시장으로 스코프를 넓힘(risk_service 자체는 기존 cn/hk/us 스코프 유지 — Toss 연동 계좌로 한정하면 자연히 kr/us만 남음). 필터: confidence≥`PHASE5_MIN_CONFIDENCE`(0~1 스케일, 기본 0.6) ∧ plan_quality 유효(`unknown`/`minimal` 제외 — 스펙 원문의 "poor"는 실제 `plan_quality` enum에 존재하지 않는 값이라 최저 등급인 `minimal`로 대체) ∧ 필수 가격필드(있다면 `stop_loss`) 유효. 수량: `sell`→원장 `PortfolioPosition.quantity` 전량, `reduce`→`floor(보유/2)`(0이면 skip), `alert`→제안 없이 요약에만 카운트. 주문 형태: `stop_loss` 있음→Phase 4 조건주문 STOP(`trigger=stop_loss`, `limit=stop_loss×(1−slippage)`, `expire=오늘+7일`), 없음→Phase 3 지정가(`get_realtime_quote` 현재가×(1−slippage), 조회 실패 시 fail-closed skip — 정규식 파싱 가격을 지정가로 쓰지 않음); 슬리피지(`PHASE5_SELL_SLIPPAGE_BPS`, 기본 50bp)는 두 경로 모두 적용(Codex 적대적 리뷰 F3/R2 수렴 이후 정의: "보호"가 아니라 두 경로가 공유하는 "체결 확실성 vs 체결가" 트레이드오프 노브 — 두 경로 모두 갭 관통 미체결 위험을 없애지 못하며, 조건주문 쪽 잔여 위험은 아래 v6/F3 accept-and-disclose로 대응). 중복 방지: 두 제안 테이블에 additive `generation_source`(manual/auto)+`source_signal_id`(nullable) 컬럼과 `source_signal_id` partial unique index(NULL 다중 허용, 비파괴 마이그레이션+레거시 DB에서 검증)를 신설해 배치 재실행을 insert-then-catch-IntegrityError로 멱등화(read-then-insert TOCTOU 아님)하고, 추가로 동일 (account,symbol,side) 활성 제안(Phase 3: pending/executing/outcome_unknown, Phase 4: pending/approving/approved/paused/registration_unknown) 존재 시에도 skip. 개별 신호 실패(한도·sellable·FX·중복 등)는 그 신호만 skip+로그, 배치는 계속. 활성화: `PHASE5_AUTO_PROPOSAL_ENABLED`(기본 false, `TOSS_ORDER_LIVE`와 동일한 엄격 파싱) + 연동 Toss 계좌 ≥1일 때만 동작, 미충족 시 no-op(로그 1줄). 배치 종료 시 기존 알림 채널로 요약 1건 전송(실패해도 이미 커밋된 제안·분석 파이프라인은 무손상). 기존 pending 목록 API(`GET .../orders/proposals`, `GET .../conditional-orders/proposals`)에 `generation_source` 필터를 additive 노출(값이 `manual`/`auto`가 아니면 422, 빈 결과 아님). **v2(Codex 독립 리뷰 BLOCK 판정 blocker 1·major 4·minor 2 반영)**: (1) 활성화를 `source_signal_id` unique index 2개의 실존 확인에 묶어 인덱스 생성 실패(기존 배포 DB 중복행 등으로 발생 가능, 로그만 남기고 DB init은 계속 성공하는 fail-open)가 배치에서는 조용히 방치되지 않고 배치 자체를 fail-closed no-op으로 거부하도록 구조화; (2) 개별 신호 격리 범위를 create_proposal 호출뿐 아니라 수량·활성 제안 조회를 포함한 신호 처리 전체(`_process_match`)로 확장; (3) 배치 시작 시 연동 계좌별 `sync_linked_account`를 계좌 단위로 격리된 best-effort로 선행 실행(ADR 0003 IP 제약·자격증명 미설정 등 sync 실패는 그 계좌만 로그 남기고 기존 원장으로 진행); (4) 활성 제안 판정에서 TTL 만료됐지만 아직 조회되지 않아 로컬 status가 `expired`로 넘어가지 않은 `pending` 제안을 비활성으로 처리; (5) 생성/스킵/alert 전부 0건이어도 배치가 실행됐다면 요약 알림 1건 전송(미실행과 구분, 인덱스 부재로 인한 refuse는 별도 error 등급 알림으로 구분). **v3(Codex 2차 독립 리뷰 BLOCK 판정 blocker 2건 반영 — v2가 도입한 인덱스 게이트 자체의 결함)**: (B1-a) 인덱스 생성 실패 catch가 `OperationalError`만 잡아, 중복 `source_signal_id` 행이 있는 기존 DB에서 SQLite `CREATE UNIQUE INDEX`가 실제로 던지는 `IntegrityError`가 `DatabaseManager` 초기화 호출부까지 전파돼 **DB 초기화 전체가 깨지던 결함**을 수정(`IntegrityError`/`OperationalError` 둘 다 catch, init은 항상 생존하고 인덱스 부재는 이후 게이트가 구조적으로 감지); (B1-b) 활성화 게이트 체커가 인덱스 **이름만** 확인해 동일 이름의 비-unique·non-partial 인덱스가 있으면 `CREATE UNIQUE INDEX IF NOT EXISTS`가 no-op인데도 체커가 True를 반환해 실제 유니크 제약 없이 배치가 활성화되던 결함을 수정(`sqlite_master`에 저장된 실제 DDL을 `UNIQUE`·대상 컬럼·`WHERE source_signal_id IS NOT NULL` predicate 세 요소로 정규식 검증). 두 수정 모두 실제 SQLite DB(중복행 DB의 init 생존, 이름만 같은 비-unique 인덱스 우회 탐지)로 실증, 일반주문/조건주문 두 테이블 모두 IntegrityError 경로 테스트 추가. **v4(Codex 3차 독립 리뷰 BLOCK 판정 blocker 1건 반영 — v3의 B1-b 수정인 DDL 정규식 파싱 자체가 취약함이 실증됨)**: 정규식이 `(source_signal_id)`를 원문 아무 위치에서나 매칭해 SQL 주석 속 동일 문자열이나 실제로는 다른/추가 컬럼에 걸린 복합 unique 인덱스를 유효로 오판하는 fail-open, 그리고 `("source_signal_id")` 같은 정상적인 SQLite 인용 식별자를 거부해 제약이 올바른 DB를 영구 fail-closed로 만드는 false-negative가 모두 실측됨 — DDL 텍스트 파싱을 전부 제거하고 `Inspector.get_indexes(table)`의 구조화 결과(`unique is True` AND `column_names == ["source_signal_id"]`, 파싱된 값이라 인용·공백·주석 무관)로 대체. `WHERE source_signal_id IS NOT NULL` partial predicate는 게이트 조건에서 의도적으로 제외(SQLite unique 인덱스는 NULL을 서로 distinct로 취급하므로 단일 컬럼 unique만으로 이미 "non-null 값당 최대 1행" 안전 속성이 보장됨). 정규식 대항 변형 3종(주석 우회·복합 인덱스·인용 식별자)을 실제 SQLite DB로 추가 검증. **v5(Codex 4차 독립 리뷰 BLOCK 판정 blocker 1건 반영 — v4의 "partial predicate 검증 불필요" 결론이 절반만 참이었음이 리뷰어 자신에 의해 정정됨)**: 그 근거(NULL distinct 취급)는 predicate가 없거나 정확히 `source_signal_id IS NOT NULL`일 때만 성립하며, `WHERE source_signal_id > 100` 같은 임의의 제한 predicate에는 적용되지 않는다 — 이런 인덱스는 unique+단일컬럼 구조 체크를 통과하면서도 predicate 밖의 행에는 유일성을 강제하지 않아 멱등성 계약을 조용히 위반한다(Codex 실측 반례). v4의 구조화 체크(unique+column_names)는 유지하되, 그 체크를 통과한 후보 인덱스 각각에 대해서만 `sqlite_master.sql`에서 자신의 실제 DDL을 조회해 WHERE 절 tail만 추출·정규화(소문자·인용부호 제거·공백 축약) 후 "predicate 없음" 또는 "정확히 `source_signal_id is not null`"일 때만 안전으로 판정하도록 복원(v3의 fail-open 전체-DDL 정규식과 달리, 이미 구조 확정된 인덱스 하나의 predicate만 좁게 확인). 제한 predicate 실제 DDL(Codex 반례 포함)로 거부 실증, 안전 predicate·non-partial 실제 DDL로 통과 실증. 설계: `docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`(v5)
- [修复] 토스증권 방어 신호 자동 주문 제안(Phase 5) Codex 적대적 리뷰(no-ship, F1·F2 blocker 통합·F3 blocker·F4 major) 4건 반영(v6): F1(다계좌 보유 시 동일 신호가 첫 계좌에만 제안됨)·F2(만료·취소된 draft의 신호가 영구히 재생성 불가)를 낳던 v1.1-v5의 단일 컬럼 `source_signal_id` partial-unique 인덱스를 **additive 컬럼 `generation_date`(KST 배치 실행일, auto만 설정) + `(account_id, source_signal_id, generation_date)` 복합 unique 인덱스(non-partial)**로 교체(옛 인덱스는 `DROP INDEX IF EXISTS` 후 새 이름으로 재생성하는 비파괴 스왑, 기존 행은 항상 더 느슨한 새 키를 만족) — 계좌별 독립 삽입으로 F1 해결, 익일부터 다른 `generation_date`로 재삽입 허용돼 F2 해결(같은 날 재시도는 여전히 억제, 의도된 동작); 게이트 체커는 `has_portfolio_proposal_source_signal_unique_indexes`에서 `has_portfolio_proposal_idempotency_unique_indexes`로 개명되고 predicate 텍스트 파싱(v3~v5의 정규식/WHERE절 검증 로직 전체)을 제거해 `Inspector.get_indexes()`의 `unique`+`column_names`(순서 고정) 구조 체크만으로 단순화(복합 키는 predicate 없이도 이미 NULL-distinct 규칙으로 manual 제안을 배제하므로 predicate 자체가 불필요해짐 — v3~v5에서 다섯 라운드에 걸쳐 발견된 predicate 파싱 취약점 클래스가 구조적으로 사라짐). F3(조건주문 stop-limit 갭다운 시 미체결)은 "고칠 수 없는" 구조적 한계로 재확인 — `PHASE5_SELL_SLIPPAGE_BPS`를 "보호"가 아닌 "체결 확실성 vs 체결가 트레이드오프 노브"로 재정의(기본값 불변)하고, **accept-and-disclose**: auto 생성 조건주문 제안의 감사 상세(`cond_proposed` 이벤트)와 조건주문이 하나라도 생성된 배치 요약 알림 양쪽에 잔여 미체결 리스크를 명시적으로 노출. F4(즉시매도 stale price)는 경로별 대응 — 신규 config `PHASE5_QUOTE_MAX_AGE_SECONDS`(기본 600초)로 생성 시점 quote에 provider 타임스탬프+최대 age를 강제(미충족 시 fail-closed skip, Phase 3 기존 `_fetch_reference_price`는 MARKET 한도 추정 전용으로 무변경)하고 채택 quote의 source/timestamp/age를 제안 감사 상세에 기록; `execute_proposal`은 `generation_source == 'auto'`인 LIMIT 매도 제안에 한해(수동 제안·조건주문 승인 경로는 완전히 무변경) 실행 직전 새 quote로 재확인해 신규 config `PHASE5_EXECUTE_PRICE_DRIFT_BPS`(기본 200bps) 이상 벌어지면 거부(제안은 `failed` 전환) — 조건주문 경로는 트리거 시 사람이 없어 이 재확인이 구조적으로 불가능하므로 F3 공시가 그 경로의 유일한 통제. 진짜 over-sell 백스톱(멱등성 인덱스와 별개)은 `execute_proposal`/`approve_proposal`의 기존 sellable 재검증이 `generation_source`와 무관하게 이미 모든 제안에 동일하게 걸린다는 것을 실측 확인 후 회귀 테스트로 고정(신규 검증 로직 없음). 두 API 응답 스키마에 `generation_date` additive 필드 노출. 설계: `docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`(v6)
- [修复] 토스증권 방어 신호 자동 주문 제안(Phase 5) F3 공시 노출면 확장(v7, coordinator 확인 하 v6 후속): v6은 조건주문 stop 제안의 갭다운 미체결 잔여 리스크 공시를 감사 상세(`cond_proposed` 이벤트)와 배치 요약 알림 두 곳에만 노출했는데, 실제 위험-수용 결정이 일어나는 승인/조회 API의 인라인 응답에는 없었던 공백을 반영 — `PortfolioConditionalOrderService._serialize_proposal`에 additive 필드 `execution_risk_disclosure`를 추가해 auto 생성 조건주문 제안에 한해 공시 문구를 채우고(manual 조건주문 제안·일반 Phase 3 주문 제안은 각각 `null`/필드 없음), create/list/get/approve/cancel 등 이 serializer를 거치는 모든 conditional-order API 응답에 자동 반영. API 스키마(`PortfolioConditionalOrderProposalItem`)에 optional 필드로 추가(하위 호환 유지, `PortfolioOrderProposalItem`은 무변경). 공시 문구 상수(`_CONDITIONAL_STOP_EXECUTION_RISK_NOTE`)는 모델/서비스 소유 모듈인 `portfolio_conditional_order_service`로 이전하고 `auto_proposal_service`가 재사용(중복 정의 제거). 설계: `docs/superpowers/specs/2026-07-20-toss-auto-proposal-phase5-design.md`(v7)
- [修复] 스케줄 일일 분석(`StockAnalysisPipeline.run`)·CLI 경로에서 접미사 없는 6자리 KR/JP 종목코드(예: `000660` = SK하이닉스)가 같은 번호의 중국 A주(`000660` = *ST南华)로 오분석되던 심각한 버그 수정: bare 6자리 KR 코드가 CN A주 코드와 문자 그대로 충돌하는데, 백엔드의 모든 시장 분류기(`_market_tag`/`detect_market`/`get_market_for_stock`/`_is_kr_market`)가 접미사 `.KS`/`.KQ`에만 의존해 bare 코드를 CN으로 기본 처리하고, CN 데이터 소스(akshare/tushare)에는 KR/JP 코드 가드가 없어 종목명·펀더멘털·실시간시세·LLM 시장맥락이 모두 엉뚱한 중국 부실주(*ST 지정·순자산 적자)로 라우팅됐다. 근인은 엔트리 경로 비대칭 — API/봇 경로는 이미 `resolve_index_stock_code_for_analysis`로 bare→`.KS` 복원을 하지만 스케줄/CLI는 하지 않았음. `process_single_stock` 진입부에 단일 choke point로 동일 resolver를 적용해 모든 엔트리(run/CLI/sync)를 커버(이미 복원된 `000660.KS`엔 idempotent, 진짜 CN `600519`·US `AAPL`은 무변경). resolver는 인덱스에 존재하는 JP/KR 접미사 종목의 유일 매치일 때만 복원하므로 CN 코드를 오변환하지 않는다. 회귀 테스트 3종 추가.
- [修复] `REPORT_LANGUAGE=ko` 시장 리뷰의 구조화 페이로드(`indices`/`sectors`/`concepts`)가 원본 中文 라벨을 그대로 실어 Web/히스토리 렌더링에서 지수·업종·테마명이 중국어로 노출되던 문제 수정(마크다운 본문은 이미 로케일화되어 있었으나 페이로드 경로만 누락). 페이로드 조립 시 지수는 안정적 `code` 기준, 업종/테마는 기존 이름 맵 기준으로 로케일화(`MarketIndex` 원본 객체는 데이터층 계약 유지). 아울러 US/HK/JP/TW 주요 지수의 한국어 라벨을 `code` 기준(`SPX`→`S&P 500 지수`, `HSI`→`항셍지수`, `N225`→`닛케이225`, `TWII`→`대만 가권지수` 등)으로 추가해, 기존에 이들 비-CN 지수가 `중국 주요 지수` 폴백으로 잘못 표기되던 문제도 함께 해결. `zh`는 무변경(no-op), 지수 code는 그대로 보존(Web/히스토리 매칭 계약 불변). 회귀 테스트 3종 추가. `en`은 기존과 동일하게 로케일화 대상이 아님(설계상 ko 전용).
- [新功能] 토스증권 서버측 조건주문(Phase 4, SINGLE/STOP만, v4 — Codex 독립 리뷰 3라운드 BLOCK 판정 반영 재확정: 1라운드 blocker 2건·major 3건·minor 1건 + 2라운드 blocker 2건·minor 1건 + 3라운드 blocker 2건·minor 1건, 3라운드는 전부 2라운드 소유권 계약의 "경쟁자 재검사~채택이 원자적이지 않다" 잔여 갈래) 추가: 로컬 상태기계 `pending → approving(원자적 claim: Phase 3 executing과 동일한 단일 write 트랜잭션 재확인+전이+한도 재검증+reservation) → approved(conditionalOrderId 보유)/registration_failed/registration_unknown`, `pending → canceled/expired/dry_run_approved`, `approved → triggered_completed/toss_expired/toss_canceled/paused(비종결)`; **승인(approve) = Toss 등록이며 등록 즉시 감시가 시작되고 트리거 시 Toss가 사용자 개입 없이 자동 체결**(Phase 3의 "자동 매매는 없다"보다 자동화 수위가 한 단계 높음을 명시적으로 수용) — dry-run(`TOSS_ORDER_LIVE` 미설정)에서는 등록 POST 자체를 하지 않는다. `TossFetcher._request_write`/신규 `_request_delete`의 live 게이트가 `/api/v1/orders*`에 더해 `/api/v1/conditional-orders*`도 path 기준(쿼리 문자열 무관)으로 차단하도록 확장하며, 두 write 경로 모두 **429에서 재시도하지 않고**(응답 유실과 동일하게 즉시 표면화 → 등록은 `registration_unknown`, 취소는 호출자 재시도) Phase 3 `place_order`의 기존 429 재시도-백오프는 무변경(회귀 테스트 다수 추가). 한도: leg 지정가×수량 기준(트리거가 아님) 추정액을 등록 시 즉시 전액 산입하며 **Phase 3 일일 한도 합산 로직에 조건주문 미확정분이 합류하는 단일 공유 상한**(조건주문 예약이 일반 주문을 차단하고 그 역도 성립, 양방향 회귀 테스트 포함)이고 1회 100만/일일 500만 KRW·1억 이상 hard reject·FX fail-closed(24시간 벽시계 포함) 등 Phase 3 안전장치를 그대로 상속; `expireDate`는 제안 생성·승인 양쪽에서 KST 오늘+7일 상한 검증. **reconcile은 Toss 조회 응답에 `clientOrderId`가 없어(§2 실측) symbol+side+트리거가+지정가+수량+만료일 속성 매칭 + Toss `createdAt`이 승인 claim 시각 −5분~현재 시간창 내 + conditionalOrderId 기준 dedupe(`OPEN`/`CLOSED` 양쪽에 걸치면 1개로 계상, `CLOSED`가 상태 우선 — v3) 후 후보 정확히 1개일 때만 채택 시도하고, 후보 0개(미발견)·2개 이상(모호) 모두 동일하게 `registration_unknown` 유지(강등도 임의 선택도 하지 않음, 모호 후보 수는 audit에 기록)**; **(v3) 유일 후보라도 곧바로 채택하지 않고 소유권을 추가 검증한다 — 그 conditionalOrderId가 이미 다른 로컬 proposal에 기록돼 있으면 후보에서 제외하고, 같은 계좌에 동일 속성이면서 아직 미해결(approving/registration_unknown)인 다른 proposal이 있으면("local contender") 채택을 보류한다(둘 다 registration_unknown 유지, candidate_count/local_contender_count/owned_by_other_proposal_count를 audit에 기록); `portfolio_conditional_order_proposals.toss_conditional_order_id`에 partial unique index(NULL 다수 허용, 비파괴 마이그레이션)를 신설해 위 애플리케이션 레벨 검사를 우회하는 경합이 실제로 발생해도 DB가 IntegrityError로 막고 registration_unknown으로 수렴시킨다(두 proposal이 같은 원격 등록을 공유하는 사고가 구조적으로 불가능해짐)**; **reconcile은 진행 중인 approve POST와 경합하지 않도록 `approving` claim이 10분 초과 경과했을 때만 원자적으로 인수하고(그 전에는 409 `approval-in-progress`), approve의 성공/명시적 4xx 전이는 `from_statuses={"approving","registration_unknown"}` 양쪽에서 허용해 어떤 인터리빙에서도 실제 POST 결과가 최종 기록으로 수렴(ID 유실 방지)**; **(v3) 조건주문 write 1회 HTTP 호출 타임아웃을 명시 상수화(15초, 401 재시도 1회 포함 최악 60초)하고, reconcile의 stale-claim 임계(10분)가 그 최악값의 10배 이상임을 모듈 import 시점 assert로 강제 — 살아있는 프로세스의 지연 POST가 이 경합 창을 통과해 유실되는 경로를 구조적으로 제거하며, 그럼에도 전이가 no-op되면(예: force-resolve 이후 지연 응답 도착) 로컬 상태는 절대 덮어쓰지 않고 audit 이벤트 `conditional_registration_conflict`(POST의 실제 outcome/ID + 로컬 최종 상태 포함)만 남겨 운영자가 Toss 측 고아/중복 등록 여부를 확인할 근거를 확보한다**. `registration_unknown`이 영구 잔류할 수 있는 가용성 한계에 대해 인증 필수 수동 종결 엔드포인트 `POST .../proposals/{uuid}/force-resolve`(body `{"confirm": true, "reason": "<필수>"}`, Toss에서 실제 미등록을 확인한 뒤에만 사용하는 운영 절차)를 신설해 `registration_unknown → registration_failed`(한도 해제) + audit `cond_force_resolved` 기록. 신규 테이블 `PortfolioConditionalOrderProposal`(additive, Phase 3 테이블·상태기계 무변경) + 기존 `PortfolioOrderAudit`(append-only, 무변경) 재사용에 `cond_*`/`conditional_registration_conflict` 이벤트 추가. API: 제안 생성/단건/목록(`GET .../conditional-orders/proposals*`), 승인(`POST .../approve`, body `confirm: true` 필수), 취소(`DELETE .../proposals/{uuid}` — pending은 로컬 취소만, approved/paused는 Toss `DELETE` 후 `toss_canceled`), 관찰 목록(`GET .../conditional-orders`, 승인된 건 lazy Toss 상태 조회 반영), 수동 일괄 동기화(`POST .../conditional-orders/sync`), force-resolve — **9개 엔드포인트 전부(조회 3종 포함)** `ADMIN_AUTH_ENABLED=true`+세션 필수(403 `order-auth-required`), OCO/OTO/`PROFIT_RATE` 요청은 스키마에 해당 필드가 없고 `extra=forbid`라 422로 거부. **설계 스펙 대비 구현 시 확정된 편차(모두 Toss 실측 OpenAPI 1.2.4 기준 검증 후 결정)**: `clientOrderId`는 스펙의 `dsa-cond-{proposal_uuid}`(45자)가 Toss의 실제 36자 상한을 넘어 전송 자체가 불가능하므로 `dc-{uuid_hex}`(35자) 사용; 조건주문 목록/단건 조회 응답에는 `clientOrderId` 필드 자체가 없어(생성 응답에만 존재) 스펙 원안의 "목록에서 clientOrderId 매칭" reconcile이 원천적으로 불가능해 위 속성+시간창+유일성 매칭으로 대체. 신규 환경변수 없음(기존 `TOSS_ORDER_*` 재사용). **잔여 리스크(v3, 명시적으로 수용)**: 사용자가 Toss 앱/API로 동일 계정에 동일 속성의 조건주문을 이 시스템의 5분 매칭 시간창 안에 수동으로 등록하는 경우는 속성만으로 구분 불가 — 실패 모드는 여전히 안전측(`registration_unknown` 유지, 오채택 없음). **(v4)** v3의 소유권·로컬 경쟁자 재검사는 일반 read session에서, 채택(상태 전이+ID 기록)은 별도 write 트랜잭션이라 그 사이에 TOCTOU 창이 남아 있었다 — 신규 `PortfolioRepository.adopt_reconciled_order_if_uncontended`가 두 재검사와 채택을 **단일 `BEGIN IMMEDIATE` write 트랜잭션**(네트워크 조회는 트랜잭션 밖에서 선행)으로 묶어, SQLite의 write 직렬화 보장으로 이 경합 창을 구조적으로 제거한다(경쟁자/소유권 충돌 발견 시 채택 취소 + `registration_unknown` 유지 + 카운트 audit 기록). 또한 `approved` 저장이 partial unique index 위반으로 실패하는 경우(그 conditionalOrderId를 다른 proposal이 이미 소유) fallback이 같은 ID를 재전달해 두 번째 `IntegrityError`로 500이 나던 결함을 고쳐, **ID 없이(NULL)** `registration_unknown`으로 전이하고 실제 ID는 `conditional_registration_conflict` audit에만 남기도록 수정(`IntegrityError`를 이 경로에서 명시적으로 catch, 다른 예외로 새지 않음) — proposal이 `approving`+500에 방치되는 일 없이 항상 reconcile/force-resolve로 복구 가능한 상태로 수렴한다. 정상 수렴 경로에도 DEBUG 로그 1줄 추가(운영 추적성, minor). 설계: docs/superpowers/specs/2026-07-19-toss-conditional-order-phase4-design.md (v4)
- [chore] 신호 귀인 후험(로드맵 D) 이연됐던 Task 3b 수동 백필 스크립트 추가: `scripts/backfill_decision_signal_attribution.py`가 캡처 이전(PR #15/#16 이전) 생성돼 `metadata_json.signal_attribution`이 없는 기존 `decision_signals` 행을 연결된 `analysis_history.raw_result`→`dashboard.signal_attribution`에서 소급 복원한다. 두 프로듀서와 동일한 공유 헬퍼(`extract_signal_attribution_for_metadata`)만 재사용(평행 구현 없음, 재정규화 없음), 원본 리포트가 없거나 파싱 실패·귀인 무효인 행은 건드리지 않고 NULL 유지. dry-run 기본값, `--apply` 명시 시에만 DB 쓰기, 이미 귀인 있는 행은 절대 덮어쓰지 않으며 raw SQL 타겟 UPDATE로 `updated_at`도 건드리지 않는다(additive-only). `decision_signal_outcomes.dominant_attribution`은 이 스크립트로 재계산되지 않음(기존 D6/D7대로 `force=true` 재평가에 위임).
...
- [新功能] 报告页「相关资讯」卡片合并 intelligence_items 池：直接查询新闻（news_intel）优先，按报告股票/市场与 `created_at` 历史时间窗补充同范围 RSS/NewsNow 池项；跨存储 URL/标题去重（直连优先）、T1>T2>T3 确定性排序、per-source 3 / market-pool 6 / direct-reserve 上限防广域 feed 淹没，`provenance`/`source`/`source_type`/`published_at` additive 元数据；`NEWS_CARD_MERGE_INTEL_ENABLED=true`（opt-out）、池查询失败 fail-open。仅读路径后处理，不新增 provider/model/base URL。
- [신규기능] 토스증권 수동 승인 반자동 주문(Phase 3, v3) 추가: 상태 기계 `pending → executing(원자적 claim: 단일 write 트랜잭션에서 재확인+전이+한도 재검증+reservation 기록) → executed/failed/outcome_unknown`, `pending → canceled/expired/dry_run_executed`; Toss POST는 reservation 커밋 성공 후에만 수행되고("로그 없으면 주문 없음"), 응답 유실·timeout·`orderId` 누락(원 실행·reconcile 재전송 양쪽 모두 outcome_unknown 유지+감사 이벤트 기록+명시적 에러 응답)·POST 후 DB 쓰기 실패·409 `request-in-progress`는 전부 `outcome_unknown`(비종결)으로 귀결되어 `POST .../proposals/{uuid}/reconcile`(동일 `clientOrderId` 재전송 — Toss 멱등)로 수렴시킨다(`idempotency-key-conflict`는 결함으로 표면화). 일일 한도(`TOSS_ORDER_DAILY_MAX_AMOUNT_KRW`, 기본 500만 KRW)는 `executing`/`outcome_unknown`을 날짜 무관 전액으로 확정 전까지 계속 산입(자정 직전 claim이 자정 직후 다른 제안의 claim-time 합산에서 누락되어 일일 한도를 우회하는 경합을 차단), 한도 검사+reservation 삽입은 SQLite `BEGIN IMMEDIATE` 단일 트랜잭션으로 동시 실행 요청을 직렬화(TOCTOU 차단); pending 10건 상한도 동일 트랜잭션으로 검사. 주문 관련 쓰기 엔드포인트 전부(제안 생성·실행·취소, 취소 주문, reconcile — dry-run 포함)는 `ADMIN_AUTH_ENABLED=true`+검증된 세션을 전제로 하며 인증 비활성 시 403 `order-auth-required`; 단일 관리자 공유 인증 시스템이므로 검증된 세션은 전 계좌를 관리하고, 자기주장 `X-Portfolio-Owner-Id` 요청 헤더 기반 owner 대조는 제공하지 않는다(검증 없는 헤더 대조는 가짜 보안). `TOSS_ORDER_LIVE`는 trim+소문자화 후 정확히 `"true"`일 때만 live(그 외 값은 ERROR 로그+dry-run 강제), `TOSS_ORDER_MAX_AMOUNT_KRW`/`TOSS_ORDER_DAILY_MAX_AMOUNT_KRW`는 NaN/Infinity/비양수를 ERROR 로그+기본값 강제로 처리. 미국 달러 등 비 KRW 주문은 환율이 미존재·stale·1:1 fallback이면 주문을 거부(fail-closed)하며, `is_stale=False`라도 실제 FX 레코드의 `updated_at`이 24시간을 초과하면 별도로 거부(벽시계 신선도는 리프레시 작업의 자체 판단과 별개 신호). `TossFetcher.cancel_order`도 `place_order`와 동일한 live 게이트를 적용하고, 내부 `_request_write`가 orders 계열 URL을 받으면 호출 경로와 무관하게(쿼리 문자열이 붙은 URL 포함 — path 기준 판정) 무조건 게이트를 강제; `place_order`는 `client_order_id`를 필수 인자로 받고 `confirmHighValueOrder`는 어떤 경로로도 전송하지 않는다(파라미터 자체 삭제). 감사 테이블 `PortfolioOrderAudit`는 SQLite 트리거로 UPDATE/DELETE를 DB 레벨에서 거부(append-only가 API 계약이 아닌 DB 불변식). 신규 테이블 2개(`PortfolioOrderProposal`, `PortfolioOrderAudit`, additive) 및 신규 환경변수 4종(전부 미설정 시 안전한 기본값으로 동작). 체결 결과는 원장에 직접 기록하지 않고 기존 Phase 2 동기화가 반영한다. 설계: docs/superpowers/specs/2026-07-17-toss-order-phase3-design.md (v3 — Codex 독립 리뷰 BLOCK 판정 8 blocker/6 major 반영 재설계 이후, 재리뷰에서 발견된 자정 경계 일일 한도 경합/FX 벽시계 신선도/자기주장 owner 헤더/URL 게이트 쿼리 문자열 우회 등 잔여 결함 추가 수정)
- [修复] 한국어 UI 잔여 중국어 문구 제거(#3): Chat/알림 센터/알림 트리거 이력/LLM 채널 설정/포트폴리오는 조사 결과 이미 CHAT_TEXT·ALERT_PAGE_TEXT·ALERT_TRIGGER_TEXT·LLM_CHANNEL_TEXT·PORTFOLIO_EXTRA_TEXT 등 기능별 사전으로 대부분 현지화되어 있었음을 확인; 남아 있던 실제 갭 2건만 수정 — 포트폴리오 스냅샷 데이터 품질 제한 사유 구분자(`snapshotQualityMessage`)와 설정 페이지 첫 실행 설정 카드 누락 항목 구분자(`FirstRunSetupCard.listSeparator`)가 ko에서 `language === 'en'` 이진 분기로 인해 중국어 전각 구두점(`；`/`、`)을 그대로 사용하던 문제를 언어별 분기로 수정. ko 렌더링 회귀 테스트 추가(PortfolioPage/SettingsPage).
- [新功能] 신호 귀인 후험(로드맵 D): (1) 캡처 — 리포트 `dashboard.signal_attribution`을 두 DecisionSignal 프로듀서 경로(일반 추출·reassess-persist)가 공유 헬퍼로 `metadata_json`에 저장(all-or-nothing, 재정규화 없음, 스키마 무변경); (2) 후험 축 — 평가 시점에 지배 귀인 라벨(`technical|news|fundamental|market|mixed`, 미귀인 NULL)을 `decision_signal_outcomes.dominant_attribution` 신규 컬럼으로 동결(SQLite 비파괴 마이그레이션+인덱스, engine_version 불변), `GET /outcomes/stats` breakdown·outcome item API·Web 신호 성과 카드(귀인 유형별 적중률, zh/en/ko)에 노출. 귀인은 후험 리포팅 전용 — 매수/매도 점수·guardrail 불변. 설계: docs/superpowers/specs/2026-07-17-signal-attribution-outcomes-design.md
- [新功能] 토스증권 포트폴리오 하이브리드 연동(Phase 2) 추가: `POST /portfolio/links/toss`(body는 `name?`/`account_seq?`만 — 계좌 자동 선택·복수 시 400+목록, 캐릭터 재사용용 `account_id` 파라미터는 제공하지 않음)로 현재 보유 종목을 합성 opening trade(trade_uid=`toss:opening:{accountSeq}:{symbol}`)로 스냅샷해 `market='kr'`/`base_currency='KRW'` 연동 계좌·opening trades·링크 행을 단일 DB 트랜잭션으로 원자적 생성하고(중간 실패 시 전부 롤백), 링크 시각 직후를 영구 `snapshot_boundary_at`으로 저장한다. `POST /portfolio/links/{account_id}/sync`는 `snapshot_boundary_at` 이후(`execution.filledAt > snapshot_boundary_at`) 체결만 반영 대상으로 삼고, 조회는 `max(snapshot_boundary_at, last_synced_at−3일)`부터 오버랩 재조회하며 `trade_uid` 유니크 제약으로 중복을 수렴시킨다(부분 체결 포함, idempotent, trade_uid=`toss:{orderId}`). 오버셀·`averageFilledPrice` 결손(주문가 대체 없음)·주문 필드 결손은 응답 `failed[]`에 기록하고 커서를 가장 이른 실패 주문 직전에 묶어 다음 sync에서 재시도한다(영구 유실 방지); 커서 DB 갱신은 monotonic. 브로커 holdings 대비 수량 드리프트는 `drift[]`로 별도 보고(원장 자동 수정 없음). `GET /portfolio/links`(활성 링크만)/`DELETE /portfolio/links/{account_id}`(링크는 `active=false`로 비활성화만 하고 커서 보존, 계좌·원장 유지) 포함 — 같은 토스 계좌로 재연동하면 비활성 링크를 재활성화해 보존된 커서에서 재개(opening 재생성 없음). KR 6자리 심볼은 기존 종목 인덱스로 `.KS`/`.KQ` 해석(미해석 시 `.KS` 기본 + 노트 표기). Toss 자격증명 미설정 시 400(`toss-not-configured`), 403(IP 미등록) 등 업스트림 실패는 502로 명시적 전달 — 조용한 성공(0건) 없음; orders 응답 envelope(`result.orders`/`hasNext`/`nextCursor`)은 엄격 검증해 스키마 이상 시 부분 결과 대신 명시적 실패. 신규 테이블 `PortfolioBrokerLink`(additive, `snapshot_boundary_at`/`active` 컬럼 포함)만 추가되고 기존 포트폴리오 테이블·엔드포인트는 무변경. 주문 생성/정정/취소 API는 호출하지 않음(읽기 전용 GET만). 신규 환경변수 없음(Phase 1 `TOSS_CLIENT_ID`/`TOSS_CLIENT_SECRET` 재사용). 설계: docs/superpowers/specs/2026-07-17-toss-portfolio-sync-design.md
- [新功能] KR 마켓 리뷰에 업종 순위(등락률 상·하위, KOSPI·KOSDAQ 독립) 연동: 무인증 DAUM sectors API 소스(`KrMarketContextFetcher.get_sector_rankings`, **Referer 필수**(누락 시 403)·`changeRate`→`change_pct` 변환·이름 중복 제거·시장·방향별 circuit breaker·동일 거래일 stale 캐시·fail-open), session은 KR 거래 캘린더 `market_phase`(XKRX)로 파생, 프롬프트 `## KR 업종 순위` 섹션(기존 '섹터 동향 데이터 없음' 대체), 결정적 본문 폭→업종→수급 순서 주입, 구조화 payload KR 전용 optional `kr_market_context.sector_rankings` 키(`breadth`와 독립), Web 구조화 카드에 KOSPI/KOSDAQ별 기존 sectors 패널 렌더(concept 패널 생성 안 함, A주 테마/개념 제외). `KR_PROFILE.has_sector_rankings` False 유지·CN 공통 `get_sector_rankings()` 경로·비KR payload/프롬프트 바이트 동일. 설계: docs/superpowers/specs/2026-07-16-kr-market-breadth-sector-design.md (#12)
- [新功能] KR 마켓 리뷰에 시장 폭(상승/하락/보합 종목 수, KOSPI·KOSDAQ 독립) 연동: 무인증 NAVER 지수 페이지 소스(`KrMarketContextFetcher`, fail-open·동일 거래일 stale 캐시·circuit breaker), 프롬프트에서 시장 폭·투자자 수급을 독립 섹션으로 분리(+일치/엇갈림 교차 해석 가이드), 결정적 본문 폭 블록 주입, 구조화 payload KR 전용 optional `kr_market_context.breadth` 키(기존 평면 breadth 계약 불변), Web 구조화 카드에 KOSPI/KOSDAQ별 3셀(상승/하락/보합) 렌더. 비KR 리뷰·Market Light·매수/매도 점수 불변. 설계: docs/superpowers/specs/2026-07-16-kr-market-breadth-sector-design.md (#11)
- [新功能] 토스증권 OpenAPI(Toss Invest) 자격증명 opt-in 데이터소스 추가: `TossFetcher`가 KR 실시간 시세 1순위(Toss→yfinance fallback)로 참여하되, KR 일봉은 yfinance(KRX 공식 종가) 1순위를 유지하고 Toss는 일봉 fallback으로만 동작(Toss 일봉 캔들은 KRX+NXT 통합 체결가라 KRX 공식 종가와 최대 3.8% 괴리 확인, 2026-07-17); US 일봉은 기존 Finnhub/AlphaVantage/YFinance/Longbridge 체인 최후순위로 참여; 실시간 quote는 `/api/v1/prices` + 최근 캔들 보충으로 change/OHLC/volume 채움(당일봉 미발행 시 pre_close만); `scripts/fetch_kr_stock_list.py`가 자격증명 존재 시 Toss 종목마스터로 한글명·상장상태 검증/보강(fail-open). `TOSS_CLIENT_ID`/`TOSS_CLIENT_SECRET` 미설정 시 기존 동작과 완전히 동일(허용 IP 등록 필요, ADR 0003).
...
- [修复] 종목 문의(Chat) 페이지·스트리밍 스토어·세션 내보내기의 잔여 하드코딩 중국어 i18n 처리: 진행 단계/도구 호출 요약/관심 종목 토스트/스트리밍 오류·빈 응답 문구를 CHAT_TEXT(zh/en/ko)로 이전, 스킬 구분자 로케일화, 다중 스킬 세션 내보내기 라벨 지원.

...
概念领跌: {bottom_concepts_text if bottom_concepts_text else "暂无数据"}"""
        ...
        if review_language == "en":
            ...
            data_boundary_requirement = (
                "- 데이터 경계 준수: 제공되지 않은 시장 폭, 자금 흐름, 거래대금, 참여 지표 또는 섹터 순위 데이터를 임의로 만들거나 과대 해석하지 마세요.\n"
                if data_limits_block
                else ""
            )
            market_summary_hint = (
                "2-3문장으로 지수, 상승/하락 종목 수, 거래대금과 시장 심리를 요약하세요"
                if self.profile.has_market_stats
                else "2-3문장으로 지수 동향, 뉴스 단서, 전반적인 리스크 상태를 요약하세요"
            )
        else:
